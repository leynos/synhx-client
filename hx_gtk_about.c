...
static void about_close(void) { gtk_widget_destroy(about_window); }
...
static void set_notebook_tab(GtkWidget *notebook, gint page_num,
  ...
  page =
      (GtkNotebookPage *)g_list_nth(GTK_NOTEBOOK(notebook)->children, page_num)
          ->data;
  notebook_page = page->child;
  gtk_widget_ref(notebook_page);
  gtk_notebook_remove_page(GTK_NOTEBOOK(notebook), page_num);
  gtk_notebook_insert_page(GTK_NOTEBOOK(notebook), notebook_page, widget,
                           page_num);
...
static char Credits[] = "\
            Programming: Misha Nasledov\n\
                        ...
                        static GdkPixmap *
                        frac_create_logo(GtkWidget *, GdkBitmap **);
...
void create_about_window(void) {
  ...
  gtk_signal_connect(GTK_OBJECT(close_button), "clicked",
                     GTK_SIGNAL_FUNC(about_close), GTK_OBJECT(about_window));
...
static unsigned int julia_loop(real_t x, real_t y, unsigned int loop_max) {
  unsigned int n;
...
static unsigned int mandelbrot_loop(real_t cx, real_t cy,
...
static unsigned int mtp(unsigned int d, real_t x) {
  unsigned int px;
...
static GdkPixmap *frac_create_logo(GtkWidget *widget, GdkBitmap **maskp) {
  GdkWindow *window;
  GdkVisual *visual;
  GdkPixmap *pixmap;
  GdkGC *gc;
  ...
  pixmap = gdk_pixmap_new(window, pixel_length[0] * 2, pixel_length[1],
                          visual->depth);
  gc = widget->style->fg_gc[GTK_STATE_NORMAL];

  gdk_draw_rgb_32_image(pixmap, gc, 0, 0, pixel_length[0], pixel_length[1],
                        GDK_RGB_DITHER_NORMAL, (guchar *)pixels,
                        pixel_length[0] * 4);

  gdk_draw_rgb_32_image(pixmap, gc, pixel_length[0], 0, pixel_length[0],
                        pixel_length[1], GDK_RGB_DITHER_NORMAL,
                        (guchar *)pixelsj, pixel_length[0] * 4);
