...
int resource_open(const char *path, int mode, int perm) {
    ...
    if (!finderinfo_path(infopath, path, &sb)) {
      ...
      for (i = 0; i < ntohs(dbl.entries); i++) {
        struct hfs_hdr_descr *descr =
            (struct hfs_hdr_descr *)(&dbl.descrs[SIZEOF_HFS_HDR_DESCR * i]);
...
static inline char *suffix(const char *path) {
...
static void suffix_type_creator(char *buf, const char *path) {
...
void type_creator(char *buf, const char *path) {
  char infopath[MAXPATHLEN];
  struct stat sb;
    ...
    if (f >= 0) {
      ...
      case HFS_FORK_DOUBLE:
        ...
        for (i = 0; i < ntohs(hdr.dbl.entries); i++) {
          struct hfs_hdr_descr *descr =
              (struct hfs_hdr_descr *)(&hdr.dbl
                                            .descrs[SIZEOF_HFS_HDR_DESCR * i]);
          if (lseek(f, ntohl(descr->offset), SEEK_SET) !=
              (off_t)ntohl(descr->offset))
            continue;
          ...
          case HFS_HDR_COMNT:
                ...
                ntohl(descr->length) > 200 ? 200 : ntohl(descr->length);
...
#define NENTRIES 4
          ...
          descr =
              (struct hfs_hdr_descr *)(&hdr.dbl
                                            .descrs[SIZEOF_HFS_HDR_DESCR * 0]);
          descr->id = htonl(HFS_HDR_COMNT);
          descr->offset =
              htonl(SIZEOF_HFS_DBL_HDR + SIZEOF_HFS_HDR_DESCR * NENTRIES);
          descr->length = htonl(fi->comlen);
          descr =
              (struct hfs_hdr_descr *)(&hdr.dbl
                                            .descrs[SIZEOF_HFS_HDR_DESCR * 1]);
          descr->id = htonl(HFS_HDR_DATES);
          descr->offset = htonl(SIZEOF_HFS_DBL_HDR +
                                SIZEOF_HFS_HDR_DESCR * NENTRIES + fi->comlen);
          descr->length = htonl(8);
          descr =
              (struct hfs_hdr_descr *)(&hdr.dbl
                                            .descrs[SIZEOF_HFS_HDR_DESCR * 2]);
