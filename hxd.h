...
extern u_int16_t nr_puts;
  ...
  extern void sql_add_user (const char *userid, const char *nick, const char *ipaddr, int port,
  const char *login, int uid, int icon, int color);
  extern void sql_modify_user(const char *nick, int icon, int color, int uid);
  extern void sql_delete_user (const char *userid, const char *nick, const char *ipaddr, int port,
  const char *login, int uid);
  extern void sql_download (const char *nick, const char *ipaddr, const char *login,
  const char *path);
  extern void sql_upload (const char *nick, const char *ipaddr, const char *login, const char *path);
  extern void sql_user_kick(const char *nick, const char *ipaddr, const char *login,
  const char *knick, const char *klogin);
  extern void sql_user_ban (const char *nick, const char *ipaddr, const char *login,
  const char *knick, const char *klogin);
...
extern int account_write (const char *login, const char *password, const char *name,
                          const struct hl_access_bits *acc);
