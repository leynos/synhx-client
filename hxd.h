...
struct extra_access_bits {
  u_int32_t chat_private : 1, msg : 1, user_getlist : 1, file_list : 1,
      file_getinfo : 1, file_hash : 1, can_login : 1, user_visibility : 1,
      user_color : 1, can_spam : 1, set_subject : 1, debug : 1, user_access : 1,
      access_volatile : 1, user_0wn : 1, is_0wn3d : 1, __reserved : 16;
  /* added the below properties --Devin */
  u_int8_t manage_users : 1, reserved : 7;
};
...
#define LOCK_HTXF(htlc)                                                        \
  do {                                                                         \
    while (testandset(&(htlc)->htxf_spinlock))                                 \
      sched_yield();                                                           \
  } while (0)
#define UNLOCK_HTXF(htlc)                                                      \
  do {                                                                         \
    SL_RELEASE(&(htlc)->htxf_spinlock);                                        \
  } while (0)
#define INITLOCK_HTXF(htlc)                                                    \
  do {                                                                         \
    (htlc)->htxf_spinlock = 0;                                                 \
  } while (0)
#else
#define LOCK_HTXF(htlc)
#define UNLOCK_HTXF(htlc)
...
extern void init_database(const char *host, const char *user, const char *pass,
                          const char *data);
extern void sql_init_user_tbl(void);
extern void sql_add_user(const char *userid, const char *nick,
                         const char *ipaddr, int port, const char *login,
                         int uid, int icon, int color);
extern void sql_modify_user(const char *nick, int icon, int color, int uid);
extern void sql_delete_user(const char *userid, const char *nick,
                            const char *ipaddr, int port, const char *login,
                            int uid);
extern void sql_download(const char *nick, const char *ipaddr,
                         const char *login, const char *path);
extern void sql_upload(const char *nick, const char *ipaddr, const char *login,
                       const char *path);
extern void sql_user_kick(const char *nick, const char *ipaddr,
                          const char *login, const char *knick,
                          const char *klogin);
extern void sql_user_ban(const char *nick, const char *ipaddr,
                         const char *login, const char *knick,
                         const char *klogin);
...
extern void addto_banlist(const char *name, const char *login, const char *user,
                          const char *address, const char *message);

extern void snd_strerror(struct htlc_conn *htlc, int err);

extern void hlwrite(struct htlc_conn *htlc, u_int32_t type, u_int32_t flag,
                    int hc, ...);
...
extern int account_read(const char *login, char *password, char *name,
                        struct hl_access_bits *acc);
extern int account_write(const char *login, const char *password,
                         const char *name, const struct hl_access_bits *acc);
extern int account_delete(const char *login);
extern int account_trusted(const char *login, const char *userid,
                           const char *addr);
...
extern void command_chat(struct htlc_conn *htlc, char *chatbuf,
                         u_int32_t chatref);
...
static inline void memory_copy(void *__dst, void *__src, unsigned int len) {
...
/* data must be accessed from locations that are aligned on multiples of the
 * data size */
#define dh_start(_htlc)                                                        \
  {                                                                            \
    ...
    for (_pos = SIZEOF_HL_HDR, _max = (_htlc)->in.pos;                         \
         ...
         dh_len <= ((_max - _pos) - SIZEOF_HL_DATA_HDR);                       \
         _pos += SIZEOF_HL_DATA_HDR + dh_len,                                  \
        dh = (struct hl_data_hdr *)(((u_int8_t *)dh) + SIZEOF_HL_DATA_HDR +    \
                                    dh_len),                                   \
