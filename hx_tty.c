...
int LI = 24, CO = 80;
char term_name[256] = "vt100", *HO, *ME, *MR, *US, *CL, *CE, *CM, *TCS, *SF,
     *SR, *DC, *IC;
...
static void term_goto_line(int line) {
  if (dumb)
    return;
...
static void term_mode_clear(void) {
  if (dumb)
    return;
...
static void term_mode_underline(void) {
  if (dumb)
    return;
...
static int term_clock_timer(char *time_str) {
  time_t t;
  struct tm tm;
...
#else
  len = sprintf(buf, "%s[HX] %s<%u>(%u) @ %s:%u", BLUE_BACK, hx_htlc.name,
                hx_htlc.icon, hx_htlc.uid, addr,
                ntohs(hx_htlc.sockaddr.SIN_PORT));
...
static RETSIGTYPE term_sigwinch(int sig __attribute__((__unused__))) {
...
static RETSIGTYPE term_sigtstp(int sig __attribute__((__unused__))) {
...
static RETSIGTYPE term_sigcont(int sig __attribute__((__unused__))) {
...
static void term_puts(char *buf, size_t len) {
  if (!dumb) {
...
static int lines(char *buf, size_t size) {
  char *p, *end = buf + size;
  int l = 0;
...
static char *line(struct screen *scr, int line) {
...
static void draw(char *ln0, size_t len) {
  tputs(CL, 0, term_putc);
  term_status();
...
static void page_up() {
  ...
  li0 =
      (scr->li > (scr->page_lines * 2)) ? (scr->li - (scr->page_lines * 2)) : 0;
  ln0 = line(scr, li0);
...
void hx_printf(struct htlc_conn *htlc, struct hx_chat *chat, const char *fmt,
  ...
  if (htlc || chat) {
  } /* removes compiler warning 'unused parameter' --Devin */
  __va_copy(save, ap);
  mal_len = 256;
  buf = scr->buf;
  for (;;) {
...
static void term_clear(struct htlc_conn *htlc, struct hx_chat *chat) {
  if (htlc || chat) {
  } /* removes compiler warning 'unused parameter' --Devin */
  if (scr_front->buf)
    xfree(scr_front->buf);
...
void hx_save(struct htlc_conn *htlc, struct hx_chat *chat,
             const char *filename) {
  int f;
  ssize_t r;
  size_t pos, len;
...
static void hotline_client_read_tty(int fd) {
  char c;
...
static void term_rl_input(char *line) {
  (void)line;
...
static int tab(int count, int key) {
  (void)count;
  (void)key;
...
static void init(int argc, char **argv) {
  ...
  if (fd == -1)
    return;
  if (fd >= hxd_open_max) {
    hxd_log("%s:%d: %d >= hxd_open_max (%d)", __FILE__, __LINE__, fd,
            hxd_open_max);
    return;
  }
...
static void cleanup() { term_reset(); }
#endif /* USE_READLINE */

static char *find_nick(char *chatbuf, char **chatstart) {
  char *nick, *p;
...
static void output_chat_str(struct htlc_conn *htlc, struct hx_chat *chat,
                            char *chatbuf, u_int16_t chatlen) {
      ...
      if (cid) {
        ...
        if (!strcmp(p, tmpstr))
          ...
          hx_printf(htlc, chat, "0x%08x: %s<%s%s%s>%s %s\n", cid, BLUE,
                    WHITE_BOLD, p, BLUE, DEFAULT, tmpstr);
      } else {
        if (!strcmp(p, tmpstr))
          hx_printf(htlc, chat, "%s%s\n", DEFAULT, tmpstr);
        else
          hx_printf(htlc, chat, "%s<%s%s%s>%s %s\n", BLUE, WHITE_BOLD, p, BLUE,
                    DEFAULT, tmpstr);
      }
    } else {
      if (cid) {
...
static void output_chat(struct htlc_conn *htlc, u_int32_t cid, char *chatbuf,
                        u_int16_t chatlen) {
  char *lastp, *p;
  int len;
  struct hx_chat *chat;
...
static void chat_invite(struct htlc_conn *htlc, u_int32_t cid, u_int32_t uid,
  ...
  hx_printf_prefix(htlc, chat, INFOPREFIX,
                   "[%s(%u)] invites you to chat 0x%08x\n", name, uid, cid);
}
...
static void output_msg(struct htlc_conn *htlc, u_int32_t uid, const char *name,
  ...
  hx_printf(htlc, 0, "%s[%s%s%s(%s%u%s)]%s: %.*s\n", WHITE, col, name, WHITE,
            ORANGE, uid, WHITE, DEFAULT, msglen, msgbuf);
}
...
static void user_create(struct htlc_conn *htlc, struct hx_chat *chat,
  ...
  if (chat->cid)
    hx_printf_prefix(htlc, 0, INFOPREFIX,
                     "chat 0x%08x join: %s%s%s [%u:%u:%u]\n", chat->cid, col,
                     nam, DEFAULT, user->uid, icon, color);
  else
    hx_printf_prefix(htlc, 0, INFOPREFIX, "join: %s%s%s [%u:%u:%u]\n", col, nam,
                     DEFAULT, user->uid, icon, color);
}
...
static void user_delete(struct htlc_conn *htlc, struct hx_chat *chat,
  ...
  if (chat->cid)
    hx_printf_prefix(htlc, 0, INFOPREFIX,
                     "chat 0x%08x parts: %s%s%s [%u:%u:%u]\n", chat->cid, col,
                     user->name, DEFAULT, user->uid, user->icon, user->color);
  else
...
static void user_change(struct htlc_conn *htlc, struct hx_chat *chat,
  ...
  if (chat->cid)
    hx_printf_prefix(
        ...
        chat->cid, col0, user->name, DEFAULT, user->uid, user->icon,
        user->color, col1, nam, DEFAULT, user->uid, icon, color);
  else
    hx_printf_prefix(htlc, 0, INFOPREFIX,
                     "%s%s%s [%u:%u:%u] is now known as %s%s%s [%u:%u:%u]\n",
                     col0, user->name, DEFAULT, user->uid, user->icon,
                     user->color, col1, nam, DEFAULT, user->uid, icon, color);
}
...
static void output_file_list(struct htlc_conn *htlc,
  ...
  for (fh = cfl->fh; (u_int32_t)((char *)fh - (char *)cfl->fh) < cfl->fhlen;
       fh = (struct hl_filelist_hdr *)((char *)fh + ntohs(fh->len) +
                                       SIZEOF_HL_DATA_HDR)) {
    ...
    hx_printf(htlc, 0, "%4.4s/%4.4s %10u %s\n", (char *)&fh->fcreator,
              (char *)&fh->ftype, ntohl(fh->fsize), buf);
  }
}
...
static void on_connect(struct htlc_conn *htlc) { hx_get_user_list(htlc, 1); }
...
struct output_functions hx_tty_output = {init,
                                         loop,
                                         cleanup,
                                         term_status,
                                         term_clear,
                                         ...
                                         on_disconnect};
#else
struct output_functions hx_tty_output = {0,
                                         0,
                                         0,
                                         0,
                                         0,
