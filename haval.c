 ...
 *                       PASS and FPTLEN replaced by fields in struct
 * haval_state.
 *
 *
...
#define FF_1(x7, x6, x5, x4, x3, x2, x1, x0, w)                                \
  {                                                                            \
    ...
    switch (state->passes) {                                                   \
    case 3:                                                                    \
      temp = f_1(x1, x0, x3, x5, x6, x2, x4);                                  \
      break;                                                                   \
    case 4:                                                                    \
      temp = f_1(x2, x6, x1, x4, x5, x3, x0);                                  \
      break;                                                                   \
    default:                                                                   \
    case 5:                                                                    \
      temp = f_1(x3, x4, x1, x0, x5, x2, x6);                                  \
      break;                                                                   \
    }                                                                          \
...
#define FF_2(x7, x6, x5, x4, x3, x2, x1, x0, w, c)                             \
  {                                                                            \
    ...
    switch (state->passes) {                                                   \
    case 3:                                                                    \
      temp = f_2(x4, x2, x1, x0, x5, x3, x6);                                  \
      break;                                                                   \
    case 4:                                                                    \
      temp = f_2(x3, x5, x2, x0, x1, x6, x4);                                  \
      break;                                                                   \
    default:                                                                   \
    case 5:                                                                    \
      temp = f_2(x6, x2, x1, x0, x3, x4, x5);                                  \
      break;                                                                   \
    }                                                                          \
...
#define FF_3(x7, x6, x5, x4, x3, x2, x1, x0, w, c)                             \
  {                                                                            \
    ...
    switch (state->passes) {                                                   \
    case 3:                                                                    \
      temp = f_3(x6, x1, x2, x3, x4, x5, x0);                                  \
      break;                                                                   \
    case 4:                                                                    \
      temp = f_3(x1, x4, x3, x6, x0, x2, x5);                                  \
      break;                                                                   \
    default:                                                                   \
    case 5:                                                                    \
      temp = f_3(x2, x6, x0, x4, x3, x1, x5);                                  \
      break;                                                                   \
    }                                                                          \
    (x7) = rotate_right(temp, 7) + rotate_right((x7), 11) + (w) + (c);         \
  }
...
#define FF_4(x7, x6, x5, x4, x3, x2, x1, x0, w, c)                             \
  {                                                                            \
    ...
    switch (state->passes) {                                                   \
    case 4:                                                                    \
      temp = f_4(x6, x4, x0, x5, x2, x1, x3);                                  \
      break;                                                                   \
    default:                                                                   \
    case 5:                                                                    \
      temp = f_4(x1, x5, x3, x2, x0, x4, x6);                                  \
      break;                                                                   \
    }                                                                          \
    (x7) = rotate_right(temp, 7) + rotate_right((x7), 11) + (w) + (c);         \
  }
...
#define ch2uint(string, word, slen)                                            \
  {                                                                            \
    ...
    while (sp < (string) + (slen)) {                                           \
      *wp++ = (haval_word) * sp | ((haval_word) * (sp + 1) << 8) |             \
              ((haval_word) * (sp + 2) << 16) |                                \
              ((haval_word) * (sp + 3) << 24);                                 \
      sp += 4;                                                                 \
    }                                                                          \
...
/* hash a buffer */
void haval_buffer(unsigned char *buf, size_t len, unsigned char *fingerprint,
                  int fptlen, int passes) {
  haval_state state;
...
int haval_fd(int fd, size_t maxlen, unsigned char *fingerprint, int fptlen,
             int passes) {
  haval_state state;
...
/* initialization */
void haval_start(haval_state *state, int fptlen, int passes) {
...
void haval_end(haval_state *state, unsigned char *final_fpt) {
  ...
  tail[0] = (unsigned char)(((state->fptlen & 0x3) << 6) |
                            ((state->passes & 0x7) << 3) | (VERSION & 0x7));
...
void haval_hash_block(haval_state *state) {
  register haval_word t0 = state->fingerprint[0], /* make use of */
      ...
      t2 = state->fingerprint[2], t3 = state->fingerprint[3],
                      t4 = state->fingerprint[4], t5 = state->fingerprint[5],
                      t6 = state->fingerprint[6], t7 = state->fingerprint[7],
                      *w = state->block;
