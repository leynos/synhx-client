...
#define SCROLLBAR_SPACING(w)                                                   \
  (GTK_SCROLLED_WINDOW_CLASS(GTK_OBJECT(w)->klass)->scrollbar_spacing)
...
extern GdkImage *cicn_to_gdkimage(GdkColormap *colormap, GdkVisual *visual,
                                  char *cicndata, unsigned int len,
                                  GdkImage **maskimp);
...
static struct pixmap_cache *pixmap_cache_look(u_int16_t icon, struct ifn *ifn,
                                              int ptype) {
...
static struct pixmap_cache *pixmap_cache_add(u_int16_t icon, GdkPixmap *pixmap,
                                             GdkBitmap *mask, int width,
                                             int height, int depth,
                                             struct ifn *ifn, int ptype) {
...
#define default_pixc                                                           \
  (load_icon(widget, DEFAULT_ICON, &user_icon_files, browser, 0))

static struct pixmap_cache *load_icon(GtkWidget *widget, u_int16_t icon,
                                      struct ifn *ifn, int browser, int ptype) {
...
found:
  ...
  image =
      cicn_to_gdkimage(colormap, visual, cicn->data, cicn->datalen, &maskim);
  if (!image)
    return default_pixc;
  depth = image->depth;
  width = image->width;
  ...
  pixc = pixmap_cache_add(icon, pixmap, mask, width - off, height, depth, ifn,
                          ptype);

  return pixc;
}

static void init_icons(struct ifn *ifn, unsigned int i) {
  int fd;
...
static void set_icon_files(struct ifn *ifn, const char *str,
                           const char *varstr) {
  const char *p;
  unsigned int i, j;
  char buf[MAXPATHLEN];
...
static GtkWidget *icon_pixmap(GtkWidget *widget, u_int16_t icon) {
  struct pixmap_cache *pixc;
  GtkWidget *gtkpixmap;
...
static GtkWidget *icon_button_new(u_int16_t iconid, const char *str,
                                  GtkWidget *widget, GtkTooltips *tooltips) {
  GtkWidget *btn;
  GtkWidget *icon;
...
static void hxd_gtk_input(gpointer data, int fd, GdkInputCondition cond) {
  if (data) {
  } /* removes compiler warning */
...
void hxd_fd_add(int fd) {
  if (fd) {
  } /* removes compiler warning */
}

void hxd_fd_del(int fd) {
  if (fd) {
  } /* removes compiler warning */
}
...
struct ghtlc_conn {
  ...
  GtkWidget *user_msgbtn, *user_infobtn, *user_kickbtn, *user_banbtn,
      *user_chatbtn;
...
static GdkColor *colorgdk(u_int16_t color) {
...
static struct gchat *gchat_with_widget(struct ghtlc_conn *ghtlc,
                                       GtkWidget *widget);
static struct ghtlc_conn *ghtlc_valid(struct ghtlc_conn *ghtlc);

static gint key_press(GtkWidget *widget, GdkEventKey *event, gpointer data) {
...
static void keyaccel_attach(struct ghtlc_conn *ghtlc, GtkWidget *widget) {
...
static void init_colors(GtkWidget *widget) {
  ...
  for (i = 0; i < 16; i++) {
    colors[i].pixel =
        (gulong)(((colors[i].red & 0xff00) << 8) + (colors[i].green & 0xff00) +
...
static struct window_geometry *wg_get(unsigned int i) {
...
static gint window_configure_event(GtkWidget *widget, GdkEventConfigure *event,
                                   gpointer data) {
...
static void window_delete_all(struct ghtlc_conn *ghtlc) {
...
static void window_delete(struct ghx_window *gwin) {
...
static struct ghx_window *window_create(struct ghtlc_conn *ghtlc,
                                        unsigned int wgi) {
  GtkWidget *window;
  struct ghx_window *gwin;
  struct window_geometry *wg;
...
static struct ghx_window *ghx_window_with_wgi(struct ghtlc_conn *ghtlc,
                                              unsigned int wgi) {
  struct ghx_window *gwin;
...
static void setbtns(struct ghtlc_conn *ghtlc, int on) {
...
static void changetitle(struct ghtlc_conn *ghtlc, GtkWidget *window,
                        char *nam) {
  char title[32];
#ifdef CONFIG_IPV6
  char addr[HOSTLEN + 1];

  inet_ntop(AFINET, (char *)&ghtlc->htlc->sockaddr.SIN_ADDR, addr,
            sizeof(addr));
#else
  char addr[16];
...
static void changetitlesconnected(struct ghtlc_conn *ghtlc) {
  struct ghx_window *gwin;
  char title[32];
#ifdef CONFIG_IPV6
  char addr[HOSTLEN + 1];
...
static void changetitlesdisconnected(struct ghtlc_conn *ghtlc) {
  struct ghx_window *gwin;
  char *title;
...
static struct ghtlc_conn *ghtlc_conn_new(struct htlc_conn *htlc) {
  struct ghtlc_conn *ghtlc;
...
static struct ghtlc_conn *ghtlc_conn_with_htlc(struct htlc_conn *htlc) {
  struct ghtlc_conn *ghtlc;
...
static void ghtlc_conn_connect(struct ghtlc_conn *ghtlc) {
  ghtlc->connected = 1;
  setbtns(ghtlc, 1);
  changetitlesconnected(ghtlc);
}

static void ghtlc_conn_disconnect(struct ghtlc_conn *ghtlc) {
  ghtlc->connected = 0;
  setbtns(ghtlc, 0);
  changetitlesdisconnected(ghtlc);
}

static struct gchat *gchat_new(struct ghtlc_conn *ghtlc, struct hx_chat *chat) {
  struct gchat *gchat;
...
static void gchat_delete(struct ghtlc_conn *ghtlc, struct gchat *gchat) {
  if (gchat->gwin) {
...
static struct gchat *gchat_with_users_list(struct ghtlc_conn *ghtlc,
                                           GtkWidget *users_list) {
  struct gchat *gchat;
...
static struct gchat *gchat_with_chat(struct ghtlc_conn *ghtlc,
                                     struct hx_chat *chat) {
  struct gchat *gchat;
...
static struct gchat *gchat_with_cid(struct ghtlc_conn *ghtlc, u_int32_t cid) {
  struct gchat *gchat;
...
static void chat_input_activate(GtkWidget *widget, gpointer data) {
...
static void chat_input_changed(GtkWidget *widget, gpointer data) {
 ...
 */
static gint chat_input_key_press(GtkWidget *widget, GdkEventKey *event,
                                 gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  GtkText *text;
  char *buf = 0, *p;
  ...
  if (k == GDK_Return) {
    ...
    if ((event->state & GDK_CONTROL_MASK)) {
      gtk_container_focus(GTK_CONTAINER(gchat->gwin->widget),
  ...
  if (line) {
    ...
    gtk_signal_connect(GTK_OBJECT(text), "changed",
                       GTK_SIGNAL_FUNC(chat_input_changed),
                       GINT_TO_POINTER(linepoint));
...
static void create_chat_text(struct ghtlc_conn *ghtlc, GtkWidget **intextp,
                             GtkWidget **outtextp, GtkWidget **vscrollbarp,
...
static void set_subject(GtkWidget *widget, gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  char *subject;
...
static void gchat_create_chat_text(struct gchat *gchat, int first) {
  GtkWidget *intext, *outtext;
  GtkWidget *vscrollbar;
  GtkWidget *subject_entry;
  GtkStyle *style;
...
static gint chat_delete_event(gpointer data) {
  ...
  if (gchat->chat && gchat->chat->cid) {
    ...
    gtk_container_remove(GTK_CONTAINER(gchat->chat_in_hbox),
                         gchat->chat_input_text);
    gtk_container_remove(GTK_CONTAINER(gchat->chat_out_hbox),
                         gchat->chat_output_text);
    gtk_container_remove(GTK_CONTAINER(gchat->chat_out_hbox),
                         gchat->chat_vscrollbar);
    gtk_container_remove(GTK_CONTAINER(gchat->subject_hbox),
                         gchat->subject_entry);
    gchat->gwin = 0;
  }
...
static GtkWidget *create_chat_widget(struct ghtlc_conn *ghtlc,
                                     struct gchat *gchat,
                                     struct window_geometry *wg) {
  GtkWidget *vbox, *hbox;
  GtkWidget *intext, *outtext;
  ...
  gtk_widget_set_usize(outputframe, (wg->width << 6) / 82,
                       (wg->height << 6) / 100);
...
static void create_chat_window(struct ghtlc_conn *ghtlc, struct hx_chat *chat) {
  struct gchat *gchat;
  struct ghx_window *gwin;
  struct window_geometry *wg;
  GtkWidget *container;
  ...
  gtk_signal_connect_object(GTK_OBJECT(window), "delete_event",
                            GTK_SIGNAL_FUNC(chat_delete_event),
                            (gpointer)gchat);
...
static void chat_subject(struct htlc_conn *htlc, u_int32_t cid,
                         const char *subject) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
...
static void chat_password(struct htlc_conn *htlc, u_int32_t cid,
                          const char *pass) {
  struct hx_chat *chat;
...
static void join_chat(GtkWidget *widget, gpointer data) {
  ...
  u_int32_t cid =
      GPOINTER_TO_INT(gtk_object_get_data(GTK_OBJECT(widget), "cid"));
  GtkWidget *window =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "dialog");

  gtk_widget_destroy(window);
  hx_chat_join(htlc, cid, 0, 0);
}

static void chat_invite(struct htlc_conn *htlc, u_int32_t cid, u_int32_t uid,
                        const char *name) {
  GtkWidget *dialog;
  GtkWidget *join;
  GtkWidget *cancel;
...
static void chat_delete(struct htlc_conn *htlc, struct hx_chat *chat) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
...
static void post_news(GtkWidget *widget, gpointer data) {
  struct ghx_window *gwin = (struct ghx_window *)data;
  struct ghtlc_conn *ghtlc =
      (struct ghtlc_conn *)gtk_object_get_data(GTK_OBJECT(widget), "ghtlc");
  GtkWidget *posttext =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "posttext");
  char *postchars;
...
static void create_post_window(struct ghtlc_conn *ghtlc) {
  struct ghx_window *gwin;
  struct window_geometry *wg;
  GtkWidget *okbut;
  GtkWidget *cancbut;
  ...
  gtk_signal_connect(GTK_OBJECT(okbut), "clicked", GTK_SIGNAL_FUNC(post_news),
                     gwin);
...
static void open_post(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  create_post_window(ghtlc);
}

static void reload_news(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  hx_get_news(ghtlc->htlc);
}

static void news_destroy(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static void create_news_window(struct ghtlc_conn *ghtlc) {
  struct ghx_window *gwin;
  struct window_geometry *wg;
  GtkWidget *window;
  GtkWidget *vscrollbar;
...
static void open_news(gpointer data) {
  ...
  if (!ghtlc->htlc->news_len)
    ...
    hx_output.news_file(ghtlc->htlc, ghtlc->htlc->news_buf,
                        ghtlc->htlc->news_len);
}

char *colorstr(u_int16_t color) {
  char *col;
...
static void connect_set_entries(struct connect_context *cc, const char *address,
...
static void server_connect(gpointer data) {
  ...
  hx_connect(htlc, serverstr, port, htlc->name, htlc->icon, login, pass,
             secure);
  g_free(serverstr);
...
static void open_bookmark(GtkWidget *widget, gpointer data) {
  struct connect_context *cc =
      (struct connect_context *)gtk_object_get_data(GTK_OBJECT(widget), "cc");
  char *file = (char *)data;
  FILE *fp;
  char line1[64];
  char line2[64];
...
static void list_bookmarks(GtkWidget *menu, struct connect_context *cc) {
  struct dirent *de;
  char *file;
...
static void bookmark_save(GtkWidget *widget, gpointer data) {
  ...
  GtkWidget *name_entry =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "name");
  GtkWidget *dialog =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "dialog");
...
static void save_dialog(gpointer data) {
  struct connect_context *cc = (struct connect_context *)data;
  GtkWidget *dialog;
  GtkWidget *ok;
  GtkWidget *cancel;
  ...
  gtk_signal_connect(GTK_OBJECT(ok), "clicked", GTK_SIGNAL_FUNC(bookmark_save),
                     cc);
...
static void close_connect_window(gpointer data) {
  struct connect_context *cc = (struct connect_context *)data;
...
static struct connect_context *create_connect_window(struct ghtlc_conn *ghtlc) {
  ...
  help_label = gtk_label_new(
      "Enter the server address, and if you have an account, your login and "
      "password. If not, leave the login and password blank.");
  ...
  gtk_signal_connect_object(GTK_OBJECT(button_cancel), "clicked",
                            GTK_SIGNAL_FUNC(close_connect_window),
                            (gpointer)cc);
...
static void open_connect(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static void list_free(void *listp) {
...
static void tracker_destroy(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static void tracker_getlist(GtkWidget *widget, gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static void tracker_server_create(struct htlc_conn *htlc, const char *addrstr,
                                  u_int16_t port, u_int16_t nusers,
...
static gint tracker_click(GtkWidget *widget, GdkEventButton *event,
                          gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  GtkWidget *tracker_list = ghtlc->tracker_list;
  struct htlc_conn *htlc;
  ...
  if (event->type == GDK_2BUTTON_PRESS) {
    ...
    hx_connect(htlc, server->addrstr, server->port, htlc->name, htlc->icon, 0,
               0, 0);
    return 1;
  }
...
static void tracker_connect(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  struct tracker_server *server;

  server =
      gtk_hlist_get_row_data(GTK_HLIST(ghtlc->tracker_list), tracker_storow);
  if (server) {
    char addrstr[32];
    struct connect_context *cc;
...
static void open_tracker(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static void change_font(struct ghtlc_conn *ghtlc, GdkFont *font) {
  struct gchat *gchat;
  GtkStyle *style;
  ...
  for (gchat = ghtlc->gchat_list; gchat; gchat = gchat->prev) {
    ...
    in_chars =
        gtk_editable_get_chars(GTK_EDITABLE(gchat->chat_input_text), 0, in_len);
    out_chars = gtk_editable_get_chars(GTK_EDITABLE(gchat->chat_output_text), 0,
                                       out_len);
    ...
    if (gchat->chat_in_hbox) {
      gtk_box_pack_start(GTK_BOX(gchat->chat_in_hbox), gchat->chat_input_text,
                         1, 1, 0);
      gtk_box_pack_start(GTK_BOX(gchat->chat_out_hbox), gchat->chat_output_text,
                         1, 1, 0);
      gtk_box_pack_start(GTK_BOX(gchat->chat_out_hbox), gchat->chat_vscrollbar,
                         0, 0, 0);
      gtk_box_pack_start(GTK_BOX(gchat->subject_hbox), gchat->subject_entry, 1,
                         1, 0);
    }
    gtk_text_insert(GTK_TEXT(gchat->chat_input_text), 0, 0, 0, in_chars,
                    in_len);
    gtk_text_insert(GTK_TEXT(gchat->chat_output_text), 0, 0, 0, out_chars,
                    out_len);
...
static void options_change(gpointer data) {
...
static void adjust_icon_list(struct options_context *oc, unsigned int height) {
...
static void icon_list_scrolled(GtkAdjustment *adj, gpointer data) {
  struct options_context *oc = (struct options_context *)data;

  adjust_icon_list(oc, (unsigned int)adj->page_size);
}

static void icon_row_selected(GtkWidget *widget, gint row, gint column,
...
static void fontsel_ok(GtkWidget *widget, gpointer data) {
  struct options_context *oc = (struct options_context *)data;
  GtkWidget *fontseldlg = gtk_object_get_data(GTK_OBJECT(widget), "fontseldlg");
  char *fontstr;

  fontstr = gtk_font_selection_dialog_get_font_name(
      GTK_FONT_SELECTION_DIALOG(fontseldlg));
  if (fontstr)
    gtk_entry_set_text(GTK_ENTRY(oc->font_entry), fontstr);
  gtk_widget_destroy(fontseldlg);
}

static void open_fontsel(gpointer data) {
  struct options_context *oc = (struct options_context *)data;
  GtkWidget *fontseldlg;

  fontseldlg = gtk_font_selection_dialog_new("Font");
  gtk_object_set_data(
      GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontseldlg)->ok_button),
      "fontseldlg", fontseldlg);
  gtk_signal_connect(
      GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontseldlg)->ok_button), "clicked",
      GTK_SIGNAL_FUNC(fontsel_ok), oc);
  gtk_signal_connect_object(
      GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontseldlg)->cancel_button),
      "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(fontseldlg));
  gtk_widget_show_all(fontseldlg);
}

static void create_options_window(struct ghtlc_conn *ghtlc) {
  struct ghx_window *gwin;
  GtkWidget *window;
  GtkWidget *mainbox;
  GtkWidget *maintab;
  ...
  gtk_table_attach(GTK_TABLE(optiontable2), showchange, 0, 1, 2, 3,
                   (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(GTK_FILL),
                   0, 0);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showjoin),
                               tty_show_user_joins);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showpart),
                               tty_show_user_parts);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showchange),
                               tty_show_user_changes);
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(maintab),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(maintab), 0), general);
  ...
  vadj = (GtkWidget *)gtk_scrolled_window_get_vadjustment(
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(maintab),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(maintab), 1), icon);
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(maintab),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(maintab), 2), sound);
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(maintab),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(maintab), 3), advanced);
...
static void open_options(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static struct msgchat *msgchat_new(struct ghtlc_conn *ghtlc, u_int32_t uid) {
...
static void msgchat_delete(struct ghtlc_conn *ghtlc, struct msgchat *msgchat) {
  if (msgchat->next)
...
static struct msgchat *msgchat_with_uid(struct ghtlc_conn *ghtlc,
                                        u_int32_t uid) {
  struct msgchat *msgchat;
...
static void destroy_msgchat(gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;

  msgchat_delete(mc->ghtlc, mc);
}

static void users_send_message(gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
...
static GtkWidget *user_pixmap(GtkWidget *widget, GdkFont *font, GdkColor *color,
  ...
  gdk_window_copy_area(pixmap, users_gc, 0, 0, pixc->pixmap, 0, 0, pixc->width,
                       pixc->height);
...
static void msgchat_input_activate(GtkWidget *widget, gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
  GtkText *text;
  guint point, len;
  gchar *chars;
...
static gint msgchat_input_key_press(GtkWidget *widget, GdkEventKey *event,
                                    gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
  GtkText *text;
  guint point, len;
...
static void chat_chk_activate(GtkWidget *widget, gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
  GtkWidget *msgtext = mc->to_text;
...
static void msgwin_chat(gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
  struct ghtlc_conn *ghtlc = mc->ghtlc;
  u_int32_t uid = mc->uid;
...
static void msgwin_get_info(gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
  struct ghtlc_conn *ghtlc = mc->ghtlc;
  u_int32_t uid = mc->uid;
...
static void users_open_message(struct ghtlc_conn *ghtlc, u_int32_t uid,
                               const char *nam) {
  GtkWidget *msgwin;
  GtkWidget *thbox1;
  GtkWidget *thbox2;
  ...
  if (gchat && gchat->chat) {
    ...
    if (user) {
      ...
      pixmap = user_pixmap(msgwin, ghtlc->users_font, colorgdk(user->color),
                           icon, nam);
      if (pixmap)
...
static void context_menu_delete(struct context_menu *cmenu) {
  struct context_menu_entry *cme = cmenu->entries,
                            *cmeend = cme + cmenu->nentries;
...
static struct context_menu *context_menu_new(struct context_menu_entry *incme,
                                             guint nentries) {
  ...
  cmenu = xmalloc(sizeof(struct context_menu) +
                  nentries * sizeof(struct context_menu_entry));
  menu = gtk_menu_new();
  cmenu->menu = menu;
  ...
  for (cme = cmenu->entries; incme < incmeend; incme++, cme++) {
    ...
    if (cme->signal_func)
      ...
      hid = 0;
    if (cme->submenu)
      gtk_menu_item_set_submenu(GTK_MENU_ITEM(cme->menuitem),
                                cme->submenu->menu);
...
static void context_menu_set_data(struct context_menu *cmenu, guint i,
                                  gpointer data) {
...
static void context_menu_set_submenu(struct context_menu *cmenu, guint i,
                                     struct context_menu *submenu) {
  struct context_menu_entry *cme = cmenu->entries + i;
...
static void user_message_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct hx_user *user;
  GtkWidget *users_list;
  GList *lp;
...
static void user_info_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct hx_user *user;
  GtkWidget *users_list;
  GList *lp;
...
static void user_kick_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct hx_user *user;
  GtkWidget *users_list;
  GList *lp;
...
static void user_ban_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct hx_user *user;
  GtkWidget *users_list;
  GList *lp;
...
static void user_chat_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct hx_user *user;
  GtkWidget *users_list;
  GList *lp;
...
static void user_invite_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct gchat *bgchat;
  struct hx_user *user;
  GtkWidget *users_list;
...
static void user_edit_btn(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
  struct hx_user *user;
  GtkWidget *users_list;
  GList *lp;
...
static struct context_menu *invite_menu_new(struct ghtlc_conn *ghtlc) {
  struct context_menu *cmenu;
  ...
  for (gchat = ghtlc->gchat_list; gchat; gchat = gchat->prev) {
    if (!(gchat->chat && gchat->chat->cid))
      continue;
    cmep->name =
        g_strdup_printf("0x%x | %s", gchat->chat->cid, gchat->chat->subject);
...
static gint user_clicked(GtkWidget *widget, GdkEventButton *event,
                         gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  int row;
  int column;

  gtk_hlist_get_selection_info(GTK_HLIST(widget), event->x, event->y, &row,
                               &column);
  if (event->type == GDK_2BUTTON_PRESS && event->button == 1) {
    ...
    if (!user_menu)
      user_menu =
          context_menu_new(user_menu_entries, CM_NENTRIES(user_menu_entries));
    if (!(gchat->chat && gchat->chat->cid)) {
      if (invite_menu)
    ...
    gtk_menu_popup(GTK_MENU(user_menu->menu), 0, 0, 0, 0, event->button,
                   event->time);
    return 1;
  }
...
static void users_list_destroy(gpointer data) {
  struct gchat *gchat = (struct gchat *)data;
...
static void create_users_window(struct ghtlc_conn *ghtlc, struct gchat *gchat) {
  ...
  gtk_hlist_set_column_justification(GTK_HLIST(users_list), 0,
                                     GTK_JUSTIFY_LEFT);
  ...
  gtk_signal_connect_object(GTK_OBJECT(users_list), "destroy",
                            GTK_SIGNAL_FUNC(users_list_destroy),
                            (gpointer)gchat);
...
static void user_list(struct htlc_conn *htlc, struct hx_chat *chat) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
  struct hx_user *user;
  GtkWidget *users_list;
...
static void users_clear(struct htlc_conn *htlc, struct hx_chat *chat) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
...
static void user_create(struct htlc_conn *htlc, struct hx_chat *chat,
                        struct hx_user *user, const char *nam, u_int16_t icon,
                        u_int16_t color) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
  struct pixmap_cache *pixc;
...
static void user_delete(struct htlc_conn *htlc, struct hx_chat *chat,
                        struct hx_user *user) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
  GtkWidget *users_list;
...
static void user_change(struct htlc_conn *htlc, struct hx_chat *chat,
                        struct hx_user *user, const char *nam, u_int16_t icon,
                        u_int16_t color) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
...
static struct gfile_list *gfl_new(struct ghtlc_conn *ghtlc, GtkWidget *window,
                                  GtkWidget *hlist, char *path) {
...
static void gfl_delete(struct ghtlc_conn *ghtlc, struct gfile_list *gfl) {
  if (gfl->next)
...
static struct gfile_list *gfl_with_hlist(struct ghtlc_conn *ghtlc,
                                         GtkWidget *hlist) {
  struct gfile_list *gfl;
...
static struct gfile_list *gfl_with_path(struct ghtlc_conn *ghtlc,
                                        const char *path) {
  struct gfile_list *gfl;
...
static struct gfile_list *gfl_with_cfl(struct ghtlc_conn *ghtlc,
                                       struct cached_filelist *cfl) {
  struct gfile_list *gfl;
...
static void open_folder(struct ghtlc_conn *ghtlc, struct cached_filelist *cfl,
                        struct hl_filelist_hdr *fh) {
  char path[4096];

  if (cfl->path[0] == '/' && cfl->path[1] == 0)
    snprintf(path, sizeof(path), "%c%.*s", dir_char, (int)ntohl(fh->fnlen),
             fh->fname);
  else
    snprintf(path, sizeof(path), "%s%c%.*s", cfl->path, dir_char,
...
static void open_file(struct ghtlc_conn *ghtlc, struct cached_filelist *cfl,
                      struct hl_filelist_hdr *fh) {
  struct htxf_conn *htxf;
  char rpath[4096], lpath[4096];

  snprintf(rpath, sizeof(rpath), "%s%c%.*s", cfl->path, dir_char,
           (int)ntohl(fh->fnlen), fh->fname);
...
static void file_reload_btn(gpointer data) {
...
static void file_download_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = gfl->hlist;
  GList *lp;
...
static void filsel_ok(GtkWidget *widget, gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  GtkWidget *files_list =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "fileslist");
  GtkWidget *filsel =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "filsel");
  struct gfile_list *gfl;
  char *lpath;
  char rpath[4096];
  struct htxf_conn *htxf;
  ...
  if (!lpath)
    return;
  snprintf(rpath, sizeof(rpath), "%s%c%s", gfl->cfl->path, dir_char,
           basename(lpath));
...
static void file_upload_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = gfl->hlist;
  GtkWidget *filsel;
  ...
  g_free(title);
  gtk_object_set_data(GTK_OBJECT(GTK_FILE_SELECTION(filsel)->ok_button),
                      "fileslist", files_list);
  gtk_object_set_data(GTK_OBJECT(GTK_FILE_SELECTION(filsel)->ok_button),
                      "filsel", filsel);
  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(filsel)->ok_button),
                     "clicked", GTK_SIGNAL_FUNC(filsel_ok), ghtlc);
  gtk_signal_connect_object(
      GTK_OBJECT(GTK_FILE_SELECTION(filsel)->cancel_button), "clicked",
      GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(filsel));

  gtk_widget_show_all(filsel);
}

static void file_preview_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = gfl->hlist;
  GList *lp;
...
static void file_info_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = gfl->hlist;
  GList *lp;
...
static void file_delete_btn(GtkWidget *widget, gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  GtkWidget *files_list =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "fileslist");
  GtkWidget *dialog =
...
static void file_delete_btn_ask(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = gfl->hlist;
  GtkWidget *dialog;
...
static void files_destroy(GtkWidget *widget, gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  struct gfile_list *gfl =
      (struct gfile_list *)gtk_object_get_data(GTK_OBJECT(widget), "gfl");

  gfl_delete(ghtlc, gfl);
}

static void file_mkdir(GtkWidget *widget, gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  GtkWidget *dialog =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "dialog");
  GtkWidget *entry =
      (GtkWidget *)gtk_object_get_data(GTK_OBJECT(widget), "entry");
  char *path;
...
static void file_folder_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = gfl->hlist;
  GtkWidget *dialog;
  ...
  if (gfl->cfl->path[0] == dir_char &&
      (gfl->cfl->path[1] == dir_char || !gfl->cfl->path[1]))
  ...
  gtk_object_set_data(GTK_OBJECT(okbtn), "dialog", dialog);
  gtk_signal_connect(GTK_OBJECT(okbtn), "clicked", GTK_SIGNAL_FUNC(file_mkdir),
                     ghtlc);
  cancelbtn = gtk_button_new_with_label("Cancel");
  gtk_signal_connect_object(GTK_OBJECT(cancelbtn), "clicked",
                            GTK_SIGNAL_FUNC(gtk_widget_destroy),
...
static void file_move_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  ...
  for (lp = GTK_HLIST(files_list)->selection; lp; lp = lp->next) {
    ...
    if (fh) {
      ...
      snprintf(frompath, sizeof(frompath), "%s%c%.*s", from_gfl->cfl->path,
               dir_char, (int)ntohl(fh->fnlen), fh->fname);
      hx_file_move(ghtlc->htlc, frompath, topath);
    }
  }
}

static void file_link_btn(gpointer data) {
  struct gfile_list *gfl = (struct gfile_list *)data;
  struct gfile_list *from_gfl = gfl->from_gfl;
  struct ghtlc_conn *ghtlc = gfl->ghtlc;
  GtkWidget *files_list = from_gfl->hlist;
...
static struct context_menu *move_menu_new(struct ghtlc_conn *ghtlc,
                                          struct gfile_list *this_gfl,
                                          void (*movefn)(gpointer)) {
  struct context_menu *cmenu;
  struct context_menu_entry *cme, *cmep;
  struct gfile_list *gfl;
  unsigned int nfl = 0;
...
static gint file_clicked(GtkWidget *widget, GdkEventButton *event,
                         gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  struct gfile_list *gfl;
  int row;
  ...
  if (event->type == GDK_2BUTTON_PRESS && event->button == 1) {
    ...
    if (!file_menu)
      file_menu =
          context_menu_new(file_menu_entries, CM_NENTRIES(file_menu_entries));
    if (move_menu)
    ...
    gtk_menu_popup(GTK_MENU(file_menu->menu), 0, 0, 0, 0, event->button,
                   event->time);
    return 1;
  }
...
static gint file_key_press(GtkWidget *widget, GdkEventKey *event,
                           gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  struct gfile_list *gfl;
  struct hl_filelist_hdr *fh;
...
static void create_files_window(struct ghtlc_conn *ghtlc, char *path) {
  ...
  downloadbtn =
      icon_button_new(ICON_DOWNLOAD, "Download", files_window, tooltips);
  ...
  folderbtn =
      icon_button_new(ICON_FOLDER, "New Folder", files_window, tooltips);
  ...
  gtk_signal_connect_object(GTK_OBJECT(deletebtn), "clicked",
                            GTK_SIGNAL_FUNC(file_delete_btn_ask),
                            (gpointer)gfl);
...
char *strcasestr_len(char *haystack, char *needle, size_t len) {
...
static u_int16_t icon_of_fh(struct hl_filelist_hdr *fh) {
  u_int16_t icon;
  ...
  if (!memcmp(&fh->ftype, "fldr", 4)) {
    ...
    if (strcasestr_len(fh->fname, "DROP BOX", len) ||
      ...
      icon = ICON_FOLDER;
  } else if (!memcmp(&fh->ftype, "JPEG", 4) || !memcmp(&fh->ftype, "PNGf", 4) ||
             !memcmp(&fh->ftype, "GIFf", 4) || !memcmp(&fh->ftype, "PICT", 4))
    icon = ICON_FILE_IMAGE;
  else if (!memcmp(&fh->ftype, "MPEG", 4) || !memcmp(&fh->ftype, "MPG ", 4) ||
           !memcmp(&fh->ftype, "AVI ", 4) || !memcmp(&fh->ftype, "MooV", 4))
    icon = ICON_FILE_MOOV;
...
static void output_file_list(struct htlc_conn *htlc,
                             struct cached_filelist *cfl) {
  struct ghtlc_conn *ghtlc;
  GtkWidget *files_list;
  struct pixmap_cache *pixc;
  ...
  for (fh = cfl->fh; (u_int32_t)((char *)fh - (char *)cfl->fh) < cfl->fhlen;
    ...
    if (!pixmap)
      ...
      gtk_hlist_set_pixtext(GTK_HLIST(files_list), row, 0, namstr, 34, pixmap,
                            mask);
  }
  gtk_hlist_thaw(GTK_HLIST(files_list));
}

static void file_info(struct htlc_conn *htlc, const char *icon,
                      const char *type, const char *crea, u_int32_t size,
                      const char *name, const char *created,
                      const char *modified, const char *comment) {
  struct ghtlc_conn *ghtlc;
...
static struct gtask *gtask_with_trans(struct ghtlc_conn *ghtlc,
                                      u_int32_t trans) {
  struct gtask *gtsk;
...
static struct gtask *gtask_with_htxf(struct ghtlc_conn *ghtlc,
                                     struct htxf_conn *htxf) {
  struct gtask *gtsk;
...
static struct gtask *gtask_new(struct ghtlc_conn *ghtlc, u_int32_t trans,
                               struct htxf_conn *htxf) {
  GtkWidget *pbar;
  GtkWidget *vbox;
  GtkWidget *hbox;
...
static void gtask_delete(struct ghtlc_conn *ghtlc, struct gtask *gtsk) {
  if (ghtlc->gtask_gtklist) {
    GList *itemlist;
...
static void gtask_list_delete(struct ghtlc_conn *ghtlc) {
...
static void task_update(struct htlc_conn *htlc, struct task *tsk) {
  GtkWidget *pbar;
  GtkWidget *label;
  char taskstr[256];
  struct ghtlc_conn *ghtlc;
  ...
  label = gtsk->label;
  pbar = gtsk->pbar;
  snprintf(taskstr, sizeof(taskstr), "Task 0x%x (%s) %u/%u", tsk->trans,
...
static void tasks_destroy(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  gtask_list_delete(ghtlc);
}

static void task_stop(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  struct gtask *gtsk;
  GList *lp, *next;
  GtkWidget *listitem;
...
static void task_go(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
  struct gtask *gtsk;
  GList *lp, *next;
  GtkWidget *listitem;
...
static void open_tasks(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  create_tasks_window(ghtlc);
}

static void file_update(struct htxf_conn *htxf) {
  GtkWidget *pbar;
  GtkWidget *label;
  struct ghtlc_conn *ghtlc;
  struct gtask *gtsk;
  ...
  snprintf(str, sizeof(str), "%s  %s/%s  %s/s  ETA: %lu s  %s",
           htxf->type == XFER_GET ? "get" : "put", posstr, sizestr, bpsstr, eta,
           htxf->path);
...
static int test_bit(char *buf, int bitno) {
  char c, m;
  c = buf[bitno / 8];
  bitno = bitno % 8;
  bitno = 7 - bitno;
...
static void inverse_bit(char *buf, int bitno) {
  char *p, c, m;
  p = &buf[bitno / 8];
  c = *p;
  bitno = bitno % 8;
...
static void user_open(void *__uesp, const char *name, const char *login,
                      const char *pass, const struct hl_access_bits *access) {
  ...
  for (i = 0; i < NACCESS; i++) {
    ...
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(ues->access_widgets[i].widget), on);
  }
}

static void useredit_login_activate(GtkWidget *widget, gpointer data) {
...
static void useredit_name_activate(GtkWidget *widget, gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;
  char *name;
  size_t len;
...
static void useredit_pass_activate(GtkWidget *widget, gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;
  char *pass;
  size_t len;
...
static void useredit_chk_activate(GtkWidget *widget, gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;
  unsigned int i;
  int bitno;
...
static void useredit_save(gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;

  useredit_name_activate(ues->name_entry, data);
  useredit_pass_activate(ues->pass_entry, data);
  if (ues->ghtlc->htlc)
    hx_useredit_create(ues->ghtlc->htlc, ues->login, ues->pass, ues->name,
                       (struct hl_access_bits *)ues->access_buf);
}

static void useredit_delete(gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;
...
static void useredit_close(gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;

  gtk_widget_destroy(ues->window);
}

static void useredit_destroy(gpointer data) {
  struct useredit_session *ues = (struct useredit_session *)data;
...
static void create_useredit_window(struct ghtlc_conn *ghtlc) {
  ...
  gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(usermod_scroll),
                                        avbox);
  ...
  gtk_label_set_justify(GTK_LABEL(pass_label), GTK_JUSTIFY_LEFT);
  gtk_table_attach(GTK_TABLE(info_table), login_label, 0, 1, 0, 1, GTK_FILL,
                   GTK_FILL, 0, 0);
  gtk_table_attach(GTK_TABLE(info_table), login_entry, 1, 2, 0, 1,
                   GTK_EXPAND | GTK_FILL, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(info_table), name_label, 0, 1, 1, 2, GTK_FILL,
                   GTK_FILL, 0, 0);
  gtk_table_attach(GTK_TABLE(info_table), name_entry, 1, 2, 1, 2,
                   GTK_EXPAND | GTK_FILL, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(info_table), pass_label, 0, 1, 2, 3, GTK_FILL,
                   GTK_FILL, 0, 0);
  gtk_table_attach(GTK_TABLE(info_table), pass_entry, 1, 2, 2, 3,
                   GTK_EXPAND | GTK_FILL, 0, 0, 0);
...
static void open_useredit(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  create_useredit_window(ghtlc);
}

static void toolbar_disconnect(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;
...
static void toolbar_close(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  ghtlc_conn_delete(ghtlc);
}

static void toolbar_destroy(gpointer data) {
  struct ghtlc_conn *ghtlc = (struct ghtlc_conn *)data;

  gtk_container_remove(GTK_CONTAINER(ghtlc->toolbar_hbox), ghtlc->connectbtn);
  gtk_container_remove(GTK_CONTAINER(ghtlc->toolbar_hbox),
                       ghtlc->disconnectbtn);
...
static void quit_btn(void) { hx_exit(0); }

extern void create_about_window(void);

static void toolbar_buttons_init(struct ghtlc_conn *ghtlc, GtkWidget *window) {
  ...
  gtk_signal_connect_object(GTK_OBJECT(disconnectbtn), "clicked",
                            GTK_SIGNAL_FUNC(toolbar_disconnect),
                            (gpointer)ghtlc);
...
static void set_window_geometry(struct window_geometry *wg, const char *str,
                                const char *varstr) {
  const char *p, *strp;
  char buf[8];
  ...
  for (p = strp = str;; p++) {
    if (*p == 'x' || *p == '+' || *p == '-' || *p == 0) {
      len = (unsigned)(p - strp) >= sizeof(buf) ? sizeof(buf) - 1
                                                : (unsigned)(p - strp);
      if (!len)
        break;
      memcpy(buf, strp, len);
...
static void set_font(char **fontstr, const char *str, const char *varstr) {
  struct ghtlc_conn *ghtlc;
  GdkFont *font;
...
static void init(int argc, char **argv) {
  ...
  variable_add(default_window_geometry, set_window_geometry,
               "window_geometry\\[*\\]\\[*\\]");
  variable_add(&icon_files, set_icon_files, "icon_files\\[*\\]");
  variable_add(&user_icon_files, set_icon_files, "user_icon_files\\[*\\]");
  default_font = xstrdup("shine");
  variable_add(&default_font, set_font, "chat_font\\[*\\]\\[*\\]");
...
static void chat_output(struct gchat *gchat, const char *buf, size_t len) {
  ...
  for (p = buf; p < end; p++) {
    if (*p == '\033' && *(p + 1) == '[') {
      gtk_text_insert(GTK_TEXT(text), 0, fgcolor, bgcolor, realtext,
                      p - realtext);
      p += 2;
      i = 0;
      while (i < 2 && ((*p >= '0' && *p <= '7') || *p == ';')) {
...
void hx_printf(struct htlc_conn *htlc, struct hx_chat *chat, const char *fmt,
               ...) {
  va_list ap;
  va_list save;
...
void hx_printf_prefix(struct htlc_conn *htlc, struct hx_chat *chat,
...
static void output_user_info(struct htlc_conn *htlc, u_int32_t uid,
                             const char *nam, const char *info, u_int16_t len) {
  struct ghtlc_conn *ghtlc;
  GtkWidget *info_window;
  GtkWidget *info_text;
  GtkStyle *style;
...
void hx_save(struct htlc_conn *htlc, struct hx_chat *chat, const char *filnam) {
  struct ghtlc_conn *ghtlc;
  GtkWidget *text;
  char *chars;
  int f;
...
static void clear(struct htlc_conn *htlc, struct hx_chat *chat) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
  GtkWidget *text;
...
static void term_mode_underline() {}

static void term_mode_clear() {}

static void output_chat(struct htlc_conn *htlc, u_int32_t cid, char *chat,
                        u_int16_t chatlen) {
  struct ghtlc_conn *ghtlc;
  struct gchat *gchat;
...
static void replymessage(gpointer data) {
  struct msgchat *mc = (struct msgchat *)data;
  GtkWidget *msgtext;
  GtkWidget *vscrollbar;
  GtkWidget *msgwin;
  ...
  gtk_signal_disconnect_by_func(GTK_OBJECT(btn), GTK_SIGNAL_FUNC(replymessage),
                                mc);
...
static void output_msg(struct htlc_conn *htlc, u_int32_t uid, const char *nam,
                       const char *msgbuf, u_int16_t msglen) {
  GtkWidget *msgwin;
  GtkWidget *thbox;
  GtkWidget *vbox1;
  GtkWidget *hbox2;
...
static void output_agreement(struct htlc_conn *htlc, const char *agreement,
                             u_int16_t len) {
  hx_printf_prefix(htlc, 0, INFOPREFIX, "Agreement:\n%.*s\n", len, agreement);
}

static void output_news_file(struct htlc_conn *htlc, const char *news,
                             u_int16_t len) {
  struct ghtlc_conn *ghtlc;
...
static void output_news_post(struct htlc_conn *htlc, const char *news,
                             u_int16_t len) {
  struct ghtlc_conn *ghtlc;
  GdkColor *fgcolor, *bgcolor;
  GtkWidget *text;
...
static void on_connect(struct htlc_conn *htlc) {
  struct ghtlc_conn *ghtlc;
...
static void on_disconnect(struct htlc_conn *htlc) {
  struct ghtlc_conn *ghtlc;
...
struct output_functions hx_gtk_output = {init,
                                         loop,
                                         cleanup,
                                         status,
                                         clear,
