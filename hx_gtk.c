...
static void
create_chat_text (struct ghtlc_conn *ghtlc, GtkWidget **intextp, GtkWidget **outtextp,
                  GtkWidget **vscrollbarp,
...
static void
connect_set_entries (struct connect_context *cc, const char *address, const char *login,
                     const char *password)
{
  if (address) {
...
{
  struct connect_context *cc = (struct connect_context *)gtk_object_get_data(GTK_OBJECT(widget),
                               "cc");
  char *file = (char *)data;
  FILE *fp;
  char line1[64];
...
create_connect_window (struct ghtlc_conn *ghtlc)
{
  ...
  help_label =
    gtk_label_new("Enter the server address, and if you have an account, your login and password. If not, leave the login and password blank.");
...
open_fontsel (gpointer data)
{
  ...
  gtk_object_set_data(GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontseldlg)->ok_button), "fontseldlg",
                      fontseldlg);
...
create_options_window (struct ghtlc_conn *ghtlc)
{
  ...
  gtk_notebook_set_tab_label(GTK_NOTEBOOK(maintab), gtk_notebook_get_nth_page(GTK_NOTEBOOK (maintab),
                             0), general);
...
task_update (struct htlc_conn *htlc, struct task *tsk)
{
  ...
  label = gtsk->label;
  pbar = gtsk->pbar;
  snprintf(taskstr, sizeof(taskstr), "Task 0x%x (%s) %u/%u", tsk->trans, tsk->str ? tsk->str : "",
           pos, pos+len);
...
static void
output_msg (struct htlc_conn *htlc, u_int32_t uid, const char *nam, const char *msgbuf,
            u_int16_t msglen)
{
  GtkWidget *msgwin;
  GtkWidget *thbox;
