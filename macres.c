...
macres_res_type_list *macres_res_type_list_get(macres_res_type_list *typelist,
                                               u_int32_t ntypes,
                                               u_int32_t type) {
  u_int32_t i;
...
u_int32_t macres_file_num_res_of_type(macres_file *mrf, u_int32_t type) {
  macres_res_type_list *rtl;

  rtl = macres_res_type_list_get(mrf->res_map.res_type_list,
                                 mrf->res_map.num_res_types, type);
  if (rtl)
...
macres_res_type_list *macres_file_read_res_type_list(macres_file *mrf) {
  int fd = mrf->fd;
  u_int32_t i;
  macres_res_type_list *rtl;

  /* Seek past the 2B type list count field */
  if (lseek(fd, mrf->res_map_off + mrf->res_map.res_type_list_off + 2,
            SEEK_SET) == -1)
    return 0;
...
int macres_file_read_res_map(macres_file *mrf) {
  int fd = mrf->fd;
...
static macres_res_type_list *macres_res_type_list_new(macres_file *mrf,
                                                      u_int32_t type) {
  macres_res_type_list *rtl;
  u_int32_t i;
...
macres_res_ref_list *macres_file_read_res_ref_list(macres_file *mrf,
                                                   u_int32_t type);

macres_res_ref_list *macres_res_ref_list_new(macres_file *mrf, u_int32_t type,
                                             macres_res_type_list *rtl) {
...
macres_res *macres_res_new(const u_int8_t *name, u_int8_t namelen,
...
int macres_add_resource(macres_file *mrf, u_int32_t type, int16_t resid,
                        const u_int8_t *name, u_int8_t namelen,
                        const u_int8_t *data, u_int32_t len) {
  macres_res_type_list *rtl;
  macres_res_ref_list *rl;
  u_int32_t i;
...
macres_file *macres_file_new(void) {
  macres_file *mrf;
...
void macres_file_print(macres_file *mrf) {
  u_int32_t i;
  u_int8_t buf[4];
...
int macres_file_write_res_type_list(int fd, macres_file *mrf) {
  u_int8_t buf[8];
  u_int32_t i;
  macres_res_type_list *rtl;
...
int macres_res_ref_list_write(int fd, macres_res_ref_list *rl) {
...
int macres_file_write_res_ref_list(int fd, macres_file *mrf) {
  u_int32_t i, j, rloff;
  macres_res_ref_list *rl;
  macres_res_type_list *rtl;
  ...
  for (i = 0; i < mrf->res_map.num_res_types; i++) {
    ...
    rloff = mrf->res_map_off + mrf->res_map.res_type_list_off +
...
int macres_file_write_res_map(int fd, macres_file *mrf) {
  /* Skip to the resource map */
  if (lseek(fd, mrf->res_map_off, SEEK_SET) == -1)
    return -1;
...
int macres_file_write_resources(int fd, macres_file *mrf) {
  u_int32_t i, j;
  macres_res *mr = 0;
  macres_res_type_list *rtl;
  macres_res_ref_list *rl;
  ...
  for (i = 0; i < mrf->res_map.num_res_types; i++) {
    ...
    for (j = 0; j < rtl->num_res_of_type; j++) {
      ...
      if (rl[j].res_map_name_list_name_off != 0xffff) {
        ...
        u_int32_t off = mrf->res_map_off + mrf->res_map.res_name_list_off +
...
void macres_file_write_resources_setup(macres_file *mrf) {
...
void macres_file_write_setup(macres_file *mrf) {
...
int macres_file_write(int fd, macres_file *mrf) {
  u_int8_t buf[16];
...
macres_file *macres_file_open(int fd) {
  macres_file *mrf;
...
macres_res *macres_res_read(int fd) {
  u_int32_t len;
  macres_res *mr;
...
int macres_res_write(int fd, macres_res *mr) {
...
void macres_res_ref_list_read(int fd, macres_res_ref_list *rl) {
...
macres_res_ref_list *macres_file_read_res_ref_list(macres_file *mrf,
  ...
  rloff =
      mrf->res_map_off + mrf->res_map.res_type_list_off + rtl->res_ref_list_off;
  if (lseek(fd, rloff, SEEK_SET) == -1)
    return 0;
...
macres_res *macres_file_get_nth_res_of_type(macres_file *mrf, u_int32_t type,
                                            u_int32_t n) {
  int fd = mrf->fd;
  macres_res *mr = 0;
  macres_res_type_list *rtl;
...
macres_res *macres_file_get_resid_of_type(macres_file *mrf, u_int32_t type,
                                          int16_t resid) {
  int fd = mrf->fd;
  u_int32_t i;
  macres_res *mr = 0;
