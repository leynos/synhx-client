...
void qbuf_add(struct qbuf *q, void *buf, u_int32_t len) {
...
static RETSIGTYPE sig_alrm(int sig __attribute__((__unused__))) {}
...
void timer_add(struct timeval *tv, int (*fn)(), void *ptr) {
  ...
  if (!timer_list || (timer_list->tv.tv_sec > timer->tv.tv_sec ||
                      (timer_list->tv.tv_sec == timer->tv.tv_sec &&
                       timer_list->tv.tv_usec > timer->tv.tv_usec))) {
  ...
  for (timerp = timer_list; timerp; timerp = timerp->next) {
    if (!timerp->next || (timerp->next->tv.tv_sec > timer->tv.tv_sec ||
                          (timerp->next->tv.tv_sec == timer->tv.tv_sec &&
                           timerp->next->tv.tv_usec > timer->tv.tv_usec))) {
...
static void timer_check(struct timeval *before, struct timeval *after) {
  ...
  for (timer = timer_list; timer; timer = timer->next) {
    if (secdiff > timer->tv.tv_sec ||
      ...
      timer->expire = 1;
      timer->tv.tv_sec = timer->add_tv.tv_sec - (secdiff - timer->tv.tv_sec);
      if (usecdiff > (timer->tv.tv_usec + timer->add_tv.tv_usec)) {
        timer->tv.tv_sec -= 1;
        timer->tv.tv_usec =
            1000000 - timer->add_tv.tv_usec + timer->tv.tv_usec - usecdiff;
      } else {
        timer->tv.tv_usec =
            timer->add_tv.tv_usec + timer->tv.tv_usec - usecdiff;
      }
    } else {
...
static void set_egd_path(char **egd_pathp, const char *str) {
  int r;
...
static RETSIGTYPE sig_bus(int sig __attribute__((__unused__))) {
  hxd_log("\n\
...
static RETSIGTYPE sig_tstp(int sig __attribute__((__unused__))) {
  hxd_log("SIGTSTP");
}
