...
static void set_info_prefix(char **infpre, const char *str) {
  *infpre = xstrdup(str);
}

static void init_vars(void) {
  g_info_prefix = xstrdup(DEFAULT_INFOPREFIX);
  variable_add(&g_info_prefix, set_info_prefix, "infoprefix");
...
void expand_tilde(char *buf, const char *str) {
  const char *p = str;
  int len;
...
static int last_msg(char *buffer, int point) {
...
static void similar_str(const char *txt1, int len1, const char *txt2, int len2,
  ...
  *max = 0;
  for (p = (char *)txt1; p < end1; p++) {
    for (q = (char *)txt2; q < end2; q++) {
      for (l = 0;
           (p + l < end1) && (q + l < end2) && (tolower(p[l]) == tolower(q[l]));
           l++)
        ;
      if (l > *max) {
        *max = l;
...
static int similar(const char *txt1, int len1, const char *txt2, int len2) {
  int sum;
  int pos1, pos2, max;
  ...
  if ((sum = max)) {
    ...
    if ((pos1 + max < len1) && (pos2 + max < len2))
      sum += similar(txt1 + pos1 + max, len1 - pos1 - max, txt2 + pos2 + max,
                     len2 - pos2 - max);
  }
...
static int nick_complete(struct htlc_conn *htlc, char *buffer) {
  char nick[64];
...
char **filename_complete(char *text) {
  DIR *dir;
  struct dirent *de;
  char *filename;
  char *dirname = 0;
  ...
  while ((de = readdir(dir))) {
    ...
    if (filename_len == 0) {
      ...
      /* Otherwise, if these match up to the length of filename, then
         it is a match. */
      if ((de->d_name[0] == filename[0]) && (d_namlen >= filename_len) &&
...
static char *expand_user_name(struct htlc_conn *htlc, char *name,
                              unsigned int nlen) {
  struct hx_chat *chat;
...
int hotline_client_tab(struct htlc_conn *htlc, struct hx_chat *chat,
  ...
  if (*buffer == '/') {
    ...
    if (!(cmd_arg(1, cmd) & 0xffff))
      return point;
    if (!strncmp(cmd, "cd", 2) || !strncmp(cmd, "ln", 2) ||
        !strncmp(cmd, "ls", 2) || !strncmp(cmd, "mv", 2) ||
        !strncmp(cmd, "rm", 2) || (!strncmp(cmd, "get", 3) && cmd[3] == ' ') ||
        !strncmp(cmd, "hash", 4) || !strncmp(cmd, "finfo", 5)) {
      int argn, ambig;
    ...
    ok:
      ...
      if (ambig >= 0) {
        ...
        if (len > orig_len)
          memmove(buffer + 1 + off + orig_len, buffer + 1 + off + len,
                  len - orig_len);
        memcpy(buffer + 1 + off, buf, len);
        buffer[1 + off + len] = 0;
        point = 1 + off + len;
      }
    } else if (!strncmp(cmd, "msg", 3) || !strncmp(cmd, "info", 4) ||
               !strncmp(cmd, "kick", 4) || !strncmp(cmd, "chat", 4) ||
               !strncmp(cmd, "invite", 3) || !strncmp(cmd, "ignore", 2) ||
      ...
      if (n && *(n + len)) {
        ...
        *p = 0;
        ...
        memmove(buffer + point, buffer + 1 + off + orig_len,
                strlen(buffer + 1 + off + orig_len));
        memcpy(buffer, buf, point);
      }
    } else if (!strncmp(cmd, "exec", 4) || !strncmp(cmd, "lcd", 3) ||
               !strncmp(cmd, "load", 4) || !strncmp(cmd, "put", 3) ||
               !strncmp(cmd, "type", 4)) {
      int argn, ambig;
      u_int16_t off, len, orig_len;
      u_int32_t ol;
...
void hotline_client_input(struct htlc_conn *htlc, struct hx_chat *chat,
                          char *str) {
  if (*str) {
    if (*str == '/' && *++str && *str != '/') {
...
void chrexpand(char *str, int len) {
  char *p;
  int off;
...
int strexpand(char *str, int len) {
  char *p;
  int off;
...
int strunexpand(char *str, int slen, char *buf, int blen) {
  char *p, *bp, c;
...
static char *human_readable(u_int32_t n, char *buf, int from_block_size,
  ...
  *p = '\0';
  ...
  if (base && base <= amt) {
    ...
    do {
      ...
      amt /= base;
      tenths = r10 / base;
      rounding = (r2 < base ? 0 < r2 + rounding : 2 + (base < r2 + rounding));
      power++;
...
char *human_size(u_int32_t size, char *buf) {
  return human_readable(size, buf, 1, -1024);
}

void hx_exit(int stat) {
  hx_savevars();
  hx_output.cleanup();
  exit(stat);
}
