...
static char *remote_read_packet(int fd, struct ServerPktHeader *pkt_hdr) {
  char *data = NULL;

  if (read(fd, pkt_hdr, sizeof(struct ServerPktHeader)) ==
      sizeof(struct ServerPktHeader)) {
...
static void remote_send_packet(int fd, u_int32_t command, void *data,
...
static u_int32_t remote_get_int(int session, int cmd) {
  struct ServerPktHeader pkt_hdr;
  char *data;
  int fd, ret = 0;
...
static char *remote_get_string_pos(int session, int cmd, u_int32_t pos) {
  struct ServerPktHeader pkt_hdr;
  char *data;
  int fd;
...
int xmms_remote_get_playlist_pos(int session) {
