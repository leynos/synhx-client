...
char *string;
{
  ...
  if (history->history_stifled &&
    ...
    if (history->history_size == 0) {
      ...
      history->the_history =
          (HIST_ENTRY **)xmalloc(history->history_size * sizeof(HIST_ENTRY *));
      history->history_length = 1;
    } else {
      if (history->history_length == (history->history_size - 1)) {
        history->history_size += DEFAULT_HISTORY_GROW_SIZE;
        history->the_history = (HIST_ENTRY **)xrealloc(
            history->the_history, history->history_size * sizeof(HIST_ENTRY *));
      }
...
HIST_ENTRY *replace_history_entry(__history, which, line, data) void *__history;
int which;
char *line;
histdata_t data;
{
