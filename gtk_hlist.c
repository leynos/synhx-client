 ...
 * the hlist's voffset */
#define ROW_TOP_YPIXEL(hlist, row)                                             \
  (((hlist)->row_height * (row)) + (((row) + 1) * HELL_SPACING) +              \
   (hlist)->voffset)

/* returns the row index from a y pixel location in the
 * context of the hlist's voffset */
#define ROW_FROM_YPIXEL(hlist, y)                                              \
  (((y) - (hlist)->voffset) / ((hlist)->row_height + HELL_SPACING))

/* gives the left pixel of the given column in context of
 * the hlist's hoffset */
#define COLUMN_LEFT_XPIXEL(hlist, colnum)                                      \
  ((hlist)->column[(colnum)].area.x + (hlist)->hoffset)

/* returns the column index from a x pixel location in the
 * context of the hlist's hoffset */
static inline gint COLUMN_FROM_XPIXEL(GtkHList *hlist, gint x) {
  gint i, cx;
...
#define LIST_HEIGHT(hlist)                                                     \
  (((hlist)->row_height * ((hlist)->rows)) +                                   \
   (HELL_SPACING * ((hlist)->rows + 1)))

/* returns the total width of the list */
static inline gint LIST_WIDTH(GtkHList *hlist) {
  gint last_column;
  ...
  if (last_column >= 0)
    return (hlist->column[last_column].area.x +
            hlist->column[last_column].area.width + COLUMN_INSET +
            HELL_SPACING);
  return 0;
}

#define GTK_HLIST_CLASS_FW(_widget_)                                           \
  GTK_HLIST_CLASS(((GtkObject *)(_widget_))->klass)

/* redraw the list if it's not frozen */
#define HLIST_UNFROZEN(hlist) (((GtkHList *)(hlist))->freeze_count == 0)
#define HLIST_REFRESH(hlist)                                                   \
  G_STMT_START {                                                               \
...
static void gtk_hlist_set_arg(GtkObject *object, GtkArg *arg, guint arg_id);
static void gtk_hlist_get_arg(GtkObject *object, GtkArg *arg, guint arg_id);
...
static void gtk_hlist_draw(GtkWidget *widget, GdkRectangle *area);
static gint gtk_hlist_expose(GtkWidget *widget, GdkEventExpose *event);
static gint gtk_hlist_key_press(GtkWidget *widget, GdkEventKey *event);
static gint gtk_hlist_button_press(GtkWidget *widget, GdkEventButton *event);
static gint gtk_hlist_button_release(GtkWidget *widget, GdkEventButton *event);
static gint gtk_hlist_motion(GtkWidget *widget, GdkEventMotion *event);
...
static gint gtk_hlist_focus_in(GtkWidget *widget, GdkEventFocus *event);
static gint gtk_hlist_focus_out(GtkWidget *widget, GdkEventFocus *event);
static gint gtk_hlist_focus(GtkContainer *container,
                            GtkDirectionType direction);
static void gtk_hlist_style_set(GtkWidget *widget, GtkStyle *previous_style);
static void gtk_hlist_drag_begin(GtkWidget *widget, GdkDragContext *context);
static gint gtk_hlist_drag_motion(GtkWidget *widget, GdkDragContext *context,
                                  gint x, gint y, guint time);
static void gtk_hlist_drag_leave(GtkWidget *widget, GdkDragContext *context,
                                 guint time);
static void gtk_hlist_drag_end(GtkWidget *widget, GdkDragContext *context);
static gboolean gtk_hlist_drag_drop(GtkWidget *widget, GdkDragContext *context,
                                    gint x, gint y, guint time);
static void gtk_hlist_drag_data_get(GtkWidget *widget, GdkDragContext *context,
...
/* Selection */
static void toggle_row(GtkHList *hlist, gint row, gint column, GdkEvent *event);
static void real_select_row(GtkHList *hlist, gint row, gint column,
                            GdkEvent *event);
static void real_unselect_row(GtkHList *hlist, gint row, gint column,
                              GdkEvent *event);
static void update_extended_selection(GtkHList *hlist, gint row);
static GList *selection_find(GtkHList *hlist, gint row_number,
                             GList *row_list_element);
static void real_select_all(GtkHList *hlist);
static void real_unselect_all(GtkHList *hlist);
static void move_vertical(GtkHList *hlist, gint row, gfloat align);
static void move_horizontal(GtkHList *hlist, gint diff);
static void real_undo_selection(GtkHList *hlist);
static void fake_unselect_all(GtkHList *hlist, gint row);
static void fake_toggle_row(GtkHList *hlist, gint row);
static void resync_selection(GtkHList *hlist, GdkEvent *event);
static void sync_selection(GtkHList *hlist, gint row, gint mode);
static void set_anchor(GtkHList *hlist, gboolean add_mode, gint anchor,
                       gint undo_anchor);
...
static void extend_selection(GtkHList *hlist, GtkScrollType scroll_type,
                             gfloat position, gboolean auto_start_selection);
static gint get_selection_info(GtkHList *hlist, gint x, gint y, gint *row,
                               gint *column);

/* Scrolling */
static void move_focus_row(GtkHList *hlist, GtkScrollType scroll_type,
                           gfloat position);
static void scroll_horizontal(GtkHList *hlist, GtkScrollType scroll_type,
                              gfloat position);
static void scroll_vertical(GtkHList *hlist, GtkScrollType scroll_type,
                            gfloat position);
static void move_horizontal(GtkHList *hlist, gint diff);
static void move_vertical(GtkHList *hlist, gint row, gfloat align);
...
static gint new_column_width(GtkHList *hlist, gint column, gint *x);
static void column_auto_resize(GtkHList *hlist, GtkHListRow *hlist_row,
                               gint column, gint old_width);
static void real_resize_column(GtkHList *hlist, gint column, gint width);
static void abort_column_resize(GtkHList *hlist);
static void cell_size_request(GtkHList *hlist, GtkHListRow *hlist_row,
                              gint column, GtkRequisition *requisition);

/* Buttons */
static void column_button_create(GtkHList *hlist, gint column);
static void column_button_clicked(GtkWidget *widget, gpointer data);

/* Adjustments */
static void adjust_adjustments(GtkHList *hlist, gboolean block_resize);
...
/* Drawing */
static void get_cell_style(GtkHList *hlist, GtkHListRow *hlist_row, gint state,
                           gint column, GtkStyle **style, GdkGC **fg_gc,
                           GdkGC **bg_gc);
static gint draw_cell_pixmap(GdkWindow *window, GdkRectangle *clip_rectangle,
                             GdkGC *fg_gc, GdkGC *fill_gc, GdkPixmap *pixmap,
                             GdkBitmap *mask, gint x, gint y, gint width,
                             gint height);
static void draw_row(GtkHList *hlist, GdkRectangle *area, gint row,
                     GtkHListRow *hlist_row);
static void draw_rows(GtkHList *hlist, GdkRectangle *area);
static void hlist_refresh(GtkHList *hlist);
static void draw_drag_highlight(GtkHList *hlist, GtkHListRow *dest_row,
                                gint dest_row_number, GtkHListDragPos drag_pos);

/* Size Allocation / Requisition */
static void size_allocate_title_buttons(GtkHList *hlist);
static void size_allocate_columns(GtkHList *hlist, gboolean block_resize);
...
static void column_title_new(GtkHList *hlist, gint column, const gchar *title);
static void columns_delete(GtkHList *hlist);
static GtkHListRow *row_new(GtkHList *hlist);
static void row_delete(GtkHList *hlist, GtkHListRow *hlist_row);
static void set_cell_contents(GtkHList *hlist, GtkHListRow *hlist_row,
                              gint column, GtkHellType type, const gchar *text,
                              guint8 spacing, GdkPixmap *pixmap,
                              GdkBitmap *mask);
static gint real_insert_row(GtkHList *hlist, gint row, gchar *text[]);
static void real_remove_row(GtkHList *hlist, gint row);
static void real_clear(GtkHList *hlist);

/* Sorting */
static gint default_compare(GtkHList *hlist, gconstpointer row1,
                            gconstpointer row2);
static void real_sort_list(GtkHList *hlist);
static GList *gtk_hlist_merge(GtkHList *hlist, GList *a, GList *b);
static GList *gtk_hlist_mergesort(GtkHList *hlist, GList *list, gint num);
/* Misc */
static gboolean title_focus(GtkHList *hlist, gint dir);
static void real_row_move(GtkHList *hlist, gint source_row, gint dest_row);
static gint column_title_passive_func(GtkWidget *widget, GdkEvent *event,
                                      gpointer data);
static void drag_dest_cell(GtkHList *hlist, gint x, gint y,
...
static void gtk_hlist_class_init(GtkHListClass *klass) {
  ...
  gtk_object_add_arg_type("GtkHList::n_columns", GTK_TYPE_UINT,
                          GTK_ARG_READWRITE | GTK_ARG_CONSTRUCT_ONLY,
                          ARG_N_COLUMNS);
  gtk_object_add_arg_type("GtkHList::shadow_type", GTK_TYPE_SHADOW_TYPE,
                          GTK_ARG_READWRITE, ARG_SHADOW_TYPE);
  gtk_object_add_arg_type("GtkHList::selection_mode", GTK_TYPE_SELECTION_MODE,
                          GTK_ARG_READWRITE, ARG_SELECTION_MODE);
  gtk_object_add_arg_type("GtkHList::row_height", GTK_TYPE_UINT,
                          GTK_ARG_READWRITE, ARG_ROW_HEIGHT);
  gtk_object_add_arg_type("GtkHList::reorderable", GTK_TYPE_BOOL,
                          GTK_ARG_READWRITE, ARG_REORDERABLE);
  gtk_object_add_arg_type("GtkHList::titles_active", GTK_TYPE_BOOL,
                          GTK_ARG_READWRITE, ARG_TITLES_ACTIVE);
  gtk_object_add_arg_type("GtkHList::use_drag_icons", GTK_TYPE_BOOL,
                          GTK_ARG_READWRITE, ARG_USE_DRAG_ICONS);
  gtk_object_add_arg_type("GtkHList::sort_type", GTK_TYPE_SORT_TYPE,
                          GTK_ARG_READWRITE, ARG_SORT_TYPE);
  ...
  widget_class->set_scroll_adjustments_signal =
      gtk_signal_new("set_scroll_adjustments", GTK_RUN_LAST, object_class->type,
  ...
  hlist_signals[SELECT_ROW] =
      gtk_signal_new("select_row", GTK_RUN_FIRST, object_class->type,
                     ...
                     GTK_TYPE_INT, GTK_TYPE_INT, GTK_TYPE_GDK_EVENT);
  hlist_signals[UNSELECT_ROW] =
      gtk_signal_new("unselect_row", GTK_RUN_FIRST, object_class->type,
                     ...
                     gtk_marshal_NONE__INT_INT_POINTER, GTK_TYPE_NONE, 3,
                     GTK_TYPE_INT, GTK_TYPE_INT, GTK_TYPE_GDK_EVENT);
  hlist_signals[ROW_MOVE] = gtk_signal_new(
      "row_move", GTK_RUN_LAST, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, row_move), gtk_marshal_NONE__INT_INT,
      GTK_TYPE_NONE, 2, GTK_TYPE_INT, GTK_TYPE_INT);
  hlist_signals[CLICK_COLUMN] =
      gtk_signal_new("click_column", GTK_RUN_FIRST, object_class->type,
                     GTK_SIGNAL_OFFSET(GtkHListClass, click_column),
                     gtk_marshal_NONE__INT, GTK_TYPE_NONE, 1, GTK_TYPE_INT);
  hlist_signals[RESIZE_COLUMN] = gtk_signal_new(
      "resize_column", GTK_RUN_LAST, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, resize_column),
      gtk_marshal_NONE__INT_INT, GTK_TYPE_NONE, 2, GTK_TYPE_INT, GTK_TYPE_INT);

  hlist_signals[TOGGLE_FOCUS_ROW] = gtk_signal_new(
      "toggle_focus_row", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, toggle_focus_row),
      gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);
  hlist_signals[SELECT_ALL] = gtk_signal_new(
      "select_all", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, select_all), gtk_marshal_NONE__NONE,
      GTK_TYPE_NONE, 0);
  hlist_signals[UNSELECT_ALL] = gtk_signal_new(
      "unselect_all", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, unselect_all), gtk_marshal_NONE__NONE,
      GTK_TYPE_NONE, 0);
  hlist_signals[UNDO_SELECTION] = gtk_signal_new(
      "undo_selection", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, undo_selection), gtk_marshal_NONE__NONE,
      GTK_TYPE_NONE, 0);
  hlist_signals[START_SELECTION] = gtk_signal_new(
      "start_selection", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, start_selection), gtk_marshal_NONE__NONE,
      GTK_TYPE_NONE, 0);
  hlist_signals[END_SELECTION] = gtk_signal_new(
      "end_selection", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, end_selection), gtk_marshal_NONE__NONE,
      GTK_TYPE_NONE, 0);
  hlist_signals[TOGGLE_ADD_MODE] = gtk_signal_new(
      "toggle_add_mode", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, toggle_add_mode), gtk_marshal_NONE__NONE,
      GTK_TYPE_NONE, 0);
  hlist_signals[EXTEND_SELECTION] = gtk_signal_new(
      "extend_selection", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, extend_selection),
      gtk_marshal_NONE__ENUM_FLOAT_BOOL, GTK_TYPE_NONE, 3, GTK_TYPE_SCROLL_TYPE,
      GTK_TYPE_FLOAT, GTK_TYPE_BOOL);
  hlist_signals[SCROLL_VERTICAL] = gtk_signal_new(
      "scroll_vertical", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, scroll_vertical),
      gtk_marshal_NONE__ENUM_FLOAT, GTK_TYPE_NONE, 2, GTK_TYPE_SCROLL_TYPE,
      GTK_TYPE_FLOAT);
  hlist_signals[SCROLL_HORIZONTAL] = gtk_signal_new(
      "scroll_horizontal", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
      GTK_SIGNAL_OFFSET(GtkHListClass, scroll_horizontal),
      gtk_marshal_NONE__ENUM_FLOAT, GTK_TYPE_NONE, 2, GTK_TYPE_SCROLL_TYPE,
      GTK_TYPE_FLOAT);
  hlist_signals[ABORT_COLUMN_RESIZE] = gtk_signal_new(
      "abort_column_resize", GTK_RUN_LAST | GTK_RUN_ACTION, object_class->type,
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_Up, 0, "scroll_vertical", 2,
                               GTK_TYPE_ENUM, GTK_SCROLL_STEP_BACKWARD,
                               GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_Down, 0, "scroll_vertical", 2,
                               GTK_TYPE_ENUM, GTK_SCROLL_STEP_FORWARD,
                               GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_Page_Up, 0, "scroll_vertical",
                               2, GTK_TYPE_ENUM, GTK_SCROLL_PAGE_BACKWARD,
                               GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_Page_Down, 0, "scroll_vertical",
                               2, GTK_TYPE_ENUM, GTK_SCROLL_PAGE_FORWARD,
                               GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_Home, GDK_CONTROL_MASK,
                               "scroll_vertical", 2, GTK_TYPE_ENUM,
                               GTK_SCROLL_JUMP, GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_End, GDK_CONTROL_MASK,
                               "scroll_vertical", 2, GTK_TYPE_ENUM,
                               GTK_SCROLL_JUMP, GTK_TYPE_FLOAT, 1.0);

  gtk_binding_entry_add_signal(
      binding_set, GDK_Up, GDK_SHIFT_MASK, "extend_selection", 3, GTK_TYPE_ENUM,
      GTK_SCROLL_STEP_BACKWARD, GTK_TYPE_FLOAT, 0.0, GTK_TYPE_BOOL, TRUE);
  gtk_binding_entry_add_signal(binding_set, GDK_Down, GDK_SHIFT_MASK,
                               "extend_selection", 3, GTK_TYPE_ENUM,
                               GTK_SCROLL_STEP_FORWARD, GTK_TYPE_FLOAT, 0.0,
                               GTK_TYPE_BOOL, TRUE);
  gtk_binding_entry_add_signal(binding_set, GDK_Page_Up, GDK_SHIFT_MASK,
                               "extend_selection", 3, GTK_TYPE_ENUM,
                               GTK_SCROLL_PAGE_BACKWARD, GTK_TYPE_FLOAT, 0.0,
                               GTK_TYPE_BOOL, TRUE);
  gtk_binding_entry_add_signal(binding_set, GDK_Page_Down, GDK_SHIFT_MASK,
                               "extend_selection", 3, GTK_TYPE_ENUM,
                               GTK_SCROLL_PAGE_FORWARD, GTK_TYPE_FLOAT, 0.0,
                               GTK_TYPE_BOOL, TRUE);
  gtk_binding_entry_add_signal(
      binding_set, GDK_Home, GDK_SHIFT_MASK | GDK_CONTROL_MASK,
      "extend_selection", 3, GTK_TYPE_ENUM, GTK_SCROLL_JUMP, GTK_TYPE_FLOAT,
      0.0, GTK_TYPE_BOOL, TRUE);
  gtk_binding_entry_add_signal(
      binding_set, GDK_End, GDK_SHIFT_MASK | GDK_CONTROL_MASK,
      "extend_selection", 3, GTK_TYPE_ENUM, GTK_SCROLL_JUMP, GTK_TYPE_FLOAT,
      1.0, GTK_TYPE_BOOL, TRUE);

  gtk_binding_entry_add_signal(binding_set, GDK_Left, 0, "scroll_horizontal", 2,
                               GTK_TYPE_ENUM, GTK_SCROLL_STEP_BACKWARD,
                               GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_Right, 0, "scroll_horizontal",
                               2, GTK_TYPE_ENUM, GTK_SCROLL_STEP_FORWARD,
                               GTK_TYPE_FLOAT, 0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_Home, 0, "scroll_horizontal", 2,
                               GTK_TYPE_ENUM, GTK_SCROLL_JUMP, GTK_TYPE_FLOAT,
                               0.0);
  gtk_binding_entry_add_signal(binding_set, GDK_End, 0, "scroll_horizontal", 2,
                               GTK_TYPE_ENUM, GTK_SCROLL_JUMP, GTK_TYPE_FLOAT,
                               1.0);

  gtk_binding_entry_add_signal(binding_set, GDK_Escape, 0, "undo_selection", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_Escape, 0,
                               "abort_column_resize", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_space, 0, "toggle_focus_row",
                               0);
  gtk_binding_entry_add_signal(binding_set, GDK_space, GDK_CONTROL_MASK,
                               "toggle_add_mode", 0);
  gtk_binding_entry_add_signal(binding_set, '/', GDK_CONTROL_MASK, "select_all",
                               0);
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_Shift_R,
                               ...
                               "end_selection", 0);
  gtk_binding_entry_add_signal(
      binding_set, GDK_Shift_L,
      GDK_RELEASE_MASK | GDK_SHIFT_MASK | GDK_CONTROL_MASK, "end_selection", 0);
  gtk_binding_entry_add_signal(
      binding_set, GDK_Shift_R,
      GDK_RELEASE_MASK | GDK_SHIFT_MASK | GDK_CONTROL_MASK, "end_selection", 0);
}
...
static void gtk_hlist_get_arg(GtkObject *object, GtkArg *arg, guint arg_id) {
  ...
  case ARG_ROW_HEIGHT:
    GTK_VALUE_UINT(*arg) =
        GTK_HLIST_ROW_HEIGHT_SET(hlist) ? hlist->row_height : 0;
    break;
  case ARG_REORDERABLE:
...
/* Constructors */
void gtk_hlist_construct(GtkHList *hlist, gint columns, gchar *titles[]) {
  ...
  if (!hlist->row_mem_chunk)
    hlist->row_mem_chunk = g_mem_chunk_new(
        "hlist row mem chunk", sizeof(GtkHListRow),
        sizeof(GtkHListRow) * HLIST_OPTIMUM_SIZE, G_ALLOC_AND_FREE);

  if (!hlist->cell_mem_chunk)
    hlist->cell_mem_chunk = g_mem_chunk_new(
        "hlist cell mem chunk", sizeof(GtkHell) * columns,
        sizeof(GtkHell) * columns * HLIST_OPTIMUM_SIZE, G_ALLOC_AND_FREE);
...
void gtk_hlist_set_hadjustment(GtkHList *hlist, GtkAdjustment *adjustment) {
  ...
  if (hlist->hadjustment) {
    ...
    gtk_signal_connect(GTK_OBJECT(hlist->hadjustment), "changed",
                       (GtkSignalFunc)hadjustment_changed, (gpointer)hlist);
...
void gtk_hlist_set_vadjustment(GtkHList *hlist, GtkAdjustment *adjustment) {
  GtkAdjustment *old_adjustment;
  ...
  if (hlist->vadjustment) {
    ...
    gtk_signal_connect(GTK_OBJECT(hlist->vadjustment), "changed",
                       (GtkSignalFunc)vadjustment_changed, (gpointer)hlist);
...
void gtk_hlist_set_shadow_type(GtkHList *hlist, GtkShadowType type) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_column_title_active(GtkHList *hlist, gint column) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_column_title_passive(GtkHList *hlist, gint column) {
  GtkButton *button;
...
void gtk_hlist_set_column_title(GtkHList *hlist, gint column,
                                const gchar *title) {
  gint new_button = 0;
...
gchar *gtk_hlist_get_column_title(GtkHList *hlist, gint column) {
  g_return_val_if_fail(hlist != NULL, NULL);
  g_return_val_if_fail(GTK_IS_HLIST(hlist), NULL);
...
void gtk_hlist_set_column_widget(GtkHList *hlist, gint column,
                                 GtkWidget *widget) {
  gint new_button = 0;
  GtkWidget *old_widget;
...
GtkWidget *gtk_hlist_get_column_widget(GtkHList *hlist, gint column) {
  g_return_val_if_fail(hlist != NULL, NULL);
  g_return_val_if_fail(GTK_IS_HLIST(hlist), NULL);
...
void gtk_hlist_set_column_visibility(GtkHList *hlist, gint column,
                                     gboolean visible) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_column_resizeable(GtkHList *hlist, gint column,
                                     gboolean resizeable) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_column_auto_resize(GtkHList *hlist, gint column,
...
gint gtk_hlist_optimal_column_width(GtkHList *hlist, gint column) {
  GtkRequisition requisition;
  GList *list;
  gint width;
  ...
  for (list = hlist->row_list; list; list = list->next) {
    GTK_HLIST_CLASS_FW(hlist)->cell_size_request(hlist, GTK_HLIST_ROW(list),
                                                 column, &requisition);
...
void gtk_hlist_set_column_width(GtkHList *hlist, gint column, gint width) {
  ...
  gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[RESIZE_COLUMN], column,
                  width);
}

void gtk_hlist_set_column_min_width(GtkHList *hlist, gint column,
                                    gint min_width) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_column_max_width(GtkHList *hlist, gint column,
                                    gint max_width) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
static void column_auto_resize(GtkHList *hlist, GtkHListRow *hlist_row,
  ...
  if (hlist_row)
    GTK_HLIST_CLASS_FW(hlist)->cell_size_request(hlist, hlist_row, column,
                                                 &requisition);
  else
    requisition.width = 0;
...
static void size_allocate_title_buttons(GtkHList *hlist) {
  ...
  for (i = 0; i < last_column; i++) {
    ...
    button_allocation.width +=
        (hlist->column[i].area.width + HELL_SPACING + 2 * COLUMN_INSET);
    ...
    if (hlist->column[last_button].resizeable) {
      ...
      gdk_window_move_resize(hlist->column[last_button].window,
                             button_allocation.x - (DRAG_WIDTH / 2), 0,
                             DRAG_WIDTH, hlist->column_title_area.height);
    } else
...
static void size_allocate_columns(GtkHList *hlist, gboolean block_resize) {
  ...
  for (i = 0; i <= last_column; i++) {
    ...
    if (hlist->column[i].width_set) {
      ...
      hlist->column[i].area.width = hlist->column[i].button->requisition.width -
                                    (HELL_SPACING + (2 * COLUMN_INSET));
...
static void column_button_create(GtkHList *hlist, gint column) {
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked",
                     (GtkSignalFunc)column_button_clicked, (gpointer)hlist);
 ...
 *   gtk_hlist_set_shift
 */
GtkHellType gtk_hlist_get_cell_type(GtkHList *hlist, gint row, gint column) {
  GtkHListRow *hlist_row;
...
gint gtk_hlist_get_text(GtkHList *hlist, gint row, gint column, gchar **text) {
  GtkHListRow *hlist_row;
...
void gtk_hlist_set_pixmap(GtkHList *hlist, gint row, gint column,
...
gint gtk_hlist_get_pixmap(GtkHList *hlist, gint row, gint column,
                          GdkPixmap **pixmap, GdkBitmap **mask) {
  GtkHListRow *hlist_row;
...
gint gtk_hlist_get_pixtext(GtkHList *hlist, gint row, gint column, gchar **text,
                           guint8 *spacing, GdkPixmap **pixmap,
                           GdkBitmap **mask) {
  GtkHListRow *hlist_row;
...
void gtk_hlist_set_shift(GtkHList *hlist, gint row, gint column, gint vertical,
                         gint horizontal) {
...
static void cell_size_request(GtkHList *hlist, GtkHListRow *hlist_row,
  ...
  get_cell_style(hlist, hlist_row, GTK_STATE_NORMAL, column, &style, NULL,
                 NULL);

  switch (hlist_row->cell[column].type) {
  case GTK_HELL_TEXT:
    requisition->width = gdk_string_width(
        style->font, GTK_HELL_TEXT(hlist_row->cell[column])->text);
  ...
  case GTK_HELL_PIXTEXT:
    gdk_window_get_size(GTK_HELL_PIXTEXT(hlist_row->cell[column])->pixmap,
                        &width, &height);
    requisition->width =
        width + GTK_HELL_PIXTEXT(hlist_row->cell[column])->spacing +
        gdk_string_width(style->font,
                         GTK_HELL_TEXT(hlist_row->cell[column])->text);

    requisition->height =
        MAX(style->font->ascent + style->font->descent, height);
    break;
  case GTK_HELL_PIXMAP:
 ...
 *   gtk_hlist_clear
 */
gint gtk_hlist_prepend(GtkHList *hlist, gchar *text[]) {
  g_return_val_if_fail(hlist != NULL, -1);
  g_return_val_if_fail(GTK_IS_HLIST(hlist), -1);
...
static gint real_insert_row(GtkHList *hlist, gint row, gchar *text[]) {
  ...
  if (!hlist->rows) {
    ...
    if (GTK_HLIST_AUTO_SORT(hlist)) /* override insertion pos */
    {
      ...
      if (hlist->sort_type == GTK_SORT_ASCENDING) {
        while (row < hlist->rows &&
               hlist->compare(hlist, hlist_row, GTK_HLIST_ROW(work)) > 0) {
          row++;
          work = work->next;
        }
      } else {
    ...
    if (row == hlist->rows)
      hlist->row_list_end =
          (g_list_append(hlist->row_list_end, hlist_row))->next;
    else
...
static void real_remove_row(GtkHList *hlist, gint row) {
  ...
  if (hlist_row->state == GTK_STATE_SELECTED)
    gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], row, -1,
                    NULL);
...
static void real_row_move(GtkHList *hlist, gint source_row, gint dest_row) {
  ...
  if (source_row < 0 || source_row >= hlist->rows || dest_row < 0 ||
      dest_row >= hlist->rows || source_row == dest_row)
    return;
...
void gtk_hlist_moveto(GtkHList *hlist, gint row, gint column, gfloat row_align,
  ...
  if (hlist->hadjustment && column >= 0) {
    ...
    if (x < 0)
      ...
      gtk_adjustment_set_value(hlist->hadjustment,
...
void gtk_hlist_set_row_height(GtkHList *hlist, guint height) {
  ...
  if (GTK_WIDGET_REALIZED(hlist)) {
    if (!GTK_HLIST_ROW_HEIGHT_SET(hlist)) {
      hlist->row_height =
          (widget->style->font->ascent + widget->style->font->descent + 1);
      hlist->row_center_offset = widget->style->font->ascent + 1.5;
    } else
      hlist->row_center_offset =
          1.5 + (hlist->row_height + widget->style->font->ascent -
...
void gtk_hlist_set_row_data_full(GtkHList *hlist, gint row, gpointer data,
...
void gtk_hlist_swap_rows(GtkHList *hlist, gint row1, gint row2) {
  gint first, last;
...
void gtk_hlist_row_move(GtkHList *hlist, gint source_row, gint dest_row) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
  ...
  gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[ROW_MOVE], source_row,
                  dest_row);
}

GtkVisibility gtk_hlist_row_is_visible(GtkHList *hlist, gint row) {
  gint top;
  ...
  if ((top < 0) || ((top + hlist->row_height) >= hlist->hlist_window_height))
    return GTK_VISIBILITY_PARTIAL;

  return GTK_VISIBILITY_FULL;
}

void gtk_hlist_set_foreground(GtkHList *hlist, gint row, GdkColor *color) {
  GtkHListRow *hlist_row;
...
void gtk_hlist_set_background(GtkHList *hlist, gint row, GdkColor *color) {
  GtkHListRow *hlist_row;
 ...
 */
void gtk_hlist_set_cell_style(GtkHList *hlist, gint row, gint column,
                              GtkStyle *style) {
  GtkRequisition requisition = {0, 0};
  GtkHListRow *hlist_row;
  ...
  if (hlist_row->cell[column].style) {
    ...
    if (GTK_WIDGET_REALIZED(hlist))
      hlist_row->cell[column].style =
          gtk_style_attach(hlist_row->cell[column].style, hlist->hlist_window);
  }
...
GtkStyle *gtk_hlist_get_cell_style(GtkHList *hlist, gint row, gint column) {
  GtkHListRow *hlist_row;
...
void gtk_hlist_set_row_style(GtkHList *hlist, gint row, GtkStyle *style) {
  GtkRequisition requisition;
  GtkHListRow *hlist_row;
  gint *old_width;
  gint i;
  ...
  if (!GTK_HLIST_AUTO_RESIZE_BLOCKED(hlist)) {
    for (i = 0; i < hlist->columns; i++)
      if (hlist->column[i].auto_resize) {
        GTK_HLIST_CLASS_FW(hlist)->cell_size_request(hlist, hlist_row, i,
                                                     &requisition);
  ...
  if (hlist_row->style) {
    ...
    if (GTK_WIDGET_REALIZED(hlist))
      hlist_row->style =
          gtk_style_attach(hlist_row->style, hlist->hlist_window);
  }
...
GtkStyle *gtk_hlist_get_row_style(GtkHList *hlist, gint row) {
  GtkHListRow *hlist_row;
 ...
 */
void gtk_hlist_set_selectable(GtkHList *hlist, gint row, gboolean selectable) {
  GtkHListRow *hlist_row;
  ...
  if (!selectable && hlist_row->state == GTK_STATE_SELECTED) {
    if (hlist->anchor >= 0 && hlist->selection_mode == GTK_SELECTION_EXTENDED) {
      hlist->drag_button = 0;
      remove_grab(hlist);
...
gboolean gtk_hlist_get_selectable(GtkHList *hlist, gint row) {
...
void gtk_hlist_select_row(GtkHList *hlist, gint row, gint column) {
  ...
  gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[SELECT_ROW], row, column,
                  NULL);
}

void gtk_hlist_unselect_row(GtkHList *hlist, gint row, gint column) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
  ...
  gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], row, column,
                  NULL);
}
...
static void toggle_row(GtkHList *hlist, gint row, gint column,
                       GdkEvent *event) {
  GtkHListRow *hlist_row;
  ...
  case GTK_SELECTION_SINGLE:
    ...
    if (hlist_row->state == GTK_STATE_SELECTED) {
      gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], row,
                      column, event);
      return;
    }
  case GTK_SELECTION_BROWSE:
...
static void fake_toggle_row(GtkHList *hlist, gint row) {
  ...
  if (HLIST_UNFROZEN(hlist) &&
      gtk_hlist_row_is_visible(hlist, row) != GTK_VISIBILITY_NONE)
    GTK_HLIST_CLASS_FW(hlist)->draw_row(hlist, NULL, row, GTK_HLIST_ROW(work));
}
...
static void real_select_row(GtkHList *hlist, gint row, gint column,
  ...
  case GTK_SELECTION_BROWSE:
    ...
    while (list) {
      ...
      if (row == sel_row)
        ...
        gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], sel_row,
                        column, event);
    }
  ...
  if (!hlist->selection) {
    hlist->selection = g_list_append(hlist->selection, GINT_TO_POINTER(row));
...
static void real_unselect_row(GtkHList *hlist, gint row, gint column,
  ...
  if (hlist_row->state == GTK_STATE_SELECTED) {
    ...
    hlist->selection = g_list_remove(hlist->selection, GINT_TO_POINTER(row));
...
static void real_select_all(GtkHList *hlist) {
  ...
  case GTK_SELECTION_EXTENDED:
    ...
    if (hlist->rows &&
        ((GtkHListRow *)(hlist->row_list->data))->state != GTK_STATE_SELECTED)
  ...
  case GTK_SELECTION_MULTIPLE:
    for (i = 0, list = hlist->row_list; list; i++, list = list->next) {
      if (((GtkHListRow *)(list->data))->state == GTK_STATE_NORMAL)
        gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[SELECT_ROW], i, -1,
                        NULL);
    }
    return;
  }
...
static void real_unselect_all(GtkHList *hlist) {
  ...
  while (list) {
    ...
    list = list->next;
    gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], i, -1,
                    NULL);
  }
}
...
static void fake_unselect_all(GtkHList *hlist, gint row) {
  ...
  for (list = hlist->undo_selection; list; list = list->next) {
    ...
    if (HLIST_UNFROZEN(hlist) &&
      ...
      GTK_HLIST_CLASS_FW(hlist)->draw_row(hlist, NULL, i, GTK_HLIST_ROW(work));
  }
}
      ...
      if (row < i || row > e) {
        ...
        if (hlist_row->selectable) {
          ...
          hlist->undo_selection =
              g_list_prepend(hlist->undo_selection, GINT_TO_POINTER(row));
        }
      }
    }
  }
      ...
      if (GTK_HLIST_ROW(list)->selectable) {
        if (g_list_find(hlist->selection, GINT_TO_POINTER(i))) {
          if (GTK_HLIST_ROW(list)->state == GTK_STATE_NORMAL) {
            ...
            gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], i,
                            -1, event);
            hlist->undo_selection =
                g_list_prepend(hlist->undo_selection, GINT_TO_POINTER(i));
          }
        } else if (GTK_HLIST_ROW(list)->state == GTK_STATE_SELECTED) {
          GTK_HLIST_ROW(list)->state = GTK_STATE_NORMAL;
          hlist->undo_unselection =
              g_list_prepend(hlist->undo_unselection, GINT_TO_POINTER(i));
        }
      }
    }
  } else {
      ...
      if (GTK_HLIST_ROW(list)->selectable) {
        if (g_list_find(hlist->selection, GINT_TO_POINTER(e))) {
          if (GTK_HLIST_ROW(list)->state == GTK_STATE_NORMAL) {
            GTK_HLIST_ROW(list)->draw_me = 1;
            GTK_HLIST_ROW(list)->state = GTK_STATE_SELECTED;
            gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[UNSELECT_ROW], e,
                            -1, event);
            hlist->undo_selection =
                g_list_prepend(hlist->undo_selection, GINT_TO_POINTER(e));
          }
        } else if (GTK_HLIST_ROW(list)->state == GTK_STATE_SELECTED) {
          GTK_HLIST_ROW(list)->state = GTK_STATE_NORMAL;
          hlist->undo_unselection =
              g_list_prepend(hlist->undo_unselection, GINT_TO_POINTER(e));
        }
      }
    }
  }
...
static void gtk_hlist_realize(GtkWidget *widget) {
  ...
  /* hlist-window */
  attributes.x =
      (hlist->internal_allocation.x + widget->style->klass->xthickness);
  attributes.y =
      (hlist->internal_allocation.y + widget->style->klass->ythickness +
  ...
  for (i = 0; i < hlist->columns; i++) {
    hlist->column[i].window =
        gdk_window_new(hlist->title_window, &attributes, attributes_mask);
  ...
  if (!gdk_colormap_alloc_color(colormap, &color, 0, 1))
    g_warning("could not allocate color %x/%x/%x", color.red, color.green,
              color.blue);
  ...
  values.foreground = (widget->style->white.pixel == 0 ? widget->style->black
                                                       : widget->style->white);
  values.function = GDK_XOR;
  values.subwindow_mode = GDK_INCLUDE_INFERIORS;
  hlist->xor_gc = gdk_gc_new_with_values(widget->window, &values,
...
static void gtk_hlist_unmap(GtkWidget *widget) {
  ...
  if (GTK_WIDGET_MAPPED(widget)) {
    ...
    for (i = 0; i < hlist->columns; i++)
      if (hlist->column[i].button && GTK_WIDGET_MAPPED(hlist->column[i].button))
...
static void gtk_hlist_draw(GtkWidget *widget, GdkRectangle *area) {
  ...
  if (GTK_WIDGET_DRAWABLE(widget)) {
    ...
    gtk_draw_shadow(
        widget->style, widget->window, GTK_STATE_NORMAL, hlist->shadow_type, 0,
        0, hlist->hlist_window_width + (2 * widget->style->klass->xthickness),
        hlist->hlist_window_height + (2 * widget->style->klass->ythickness) +
            hlist->column_title_area.height);
...
static gint gtk_hlist_expose(GtkWidget *widget, GdkEventExpose *event) {
  ...
  if (GTK_WIDGET_DRAWABLE(widget)) {
    ...
    if (event->window == widget->window)
      gtk_draw_shadow(
          widget->style, widget->window, GTK_STATE_NORMAL, hlist->shadow_type,
          0, 0,
          hlist->hlist_window_width + (2 * widget->style->klass->xthickness),
          hlist->hlist_window_height + (2 * widget->style->klass->ythickness) +
              hlist->column_title_area.height);
...
static gint gtk_hlist_key_press(GtkWidget *widget, GdkEventKey *event) {
  ...
  case GDK_ISO_Left_Tab:
    if (event->state & GDK_SHIFT_MASK)
      return gtk_container_focus(GTK_CONTAINER(widget), GTK_DIR_TAB_BACKWARD);
    else
      return gtk_container_focus(GTK_CONTAINER(widget), GTK_DIR_TAB_FORWARD);
  default:
    break;
  }
  return FALSE;
  ...
  if (event->window == hlist->hlist_window) {
    ...
    if (get_selection_info(hlist, x, y, &row, &column)) {
      ...
      if (event->type == GDK_BUTTON_PRESS) {
        GdkEventMask mask =
            ((1 << (4 + event->button)) | GDK_POINTER_MOTION_HINT_MASK |
             GDK_BUTTON_RELEASE_MASK);

        if (gdk_pointer_grab(hlist->hlist_window, FALSE, mask, NULL, NULL,
                             event->time))
          return FALSE;
        gtk_grab_add(widget);
      ...
      if (button_actions & GTK_BUTTON_SELECTS) {
        ...
        case GTK_SELECTION_MULTIPLE:
          if (event->type != GDK_BUTTON_PRESS) {
            gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[SELECT_ROW], row,
                            column, event);
            hlist->anchor = -1;
          } else
            hlist->anchor = row;
        ...
        case GTK_SELECTION_EXTENDED:
          if (event->type != GDK_BUTTON_PRESS) {
            if (hlist->anchor != -1) {
              update_extended_selection(hlist, hlist->focus_row);
              GTK_HLIST_CLASS_FW(hlist)->resync_selection(hlist,
                                                          (GdkEvent *)event);
            }
            gtk_signal_emit(GTK_OBJECT(hlist), hlist_signals[SELECT_ROW], row,
                            column, event);
...
static gint gtk_hlist_button_release(GtkWidget *widget, GdkEventButton *event) {
  ...
  if (hlist->drag_button == event->button) {
    ...
    if (button_actions & GTK_BUTTON_SELECTS) {
      switch (hlist->selection_mode) {
      case GTK_SELECTION_EXTENDED:
        if (!(event->state & GDK_SHIFT_MASK) || !GTK_WIDGET_CAN_FOCUS(widget) ||
            event->x < 0 || event->x >= hlist->hlist_window_width ||
            event->y < 0 || event->y >= hlist->hlist_window_height)
          GTK_HLIST_CLASS_FW(hlist)->resync_selection(hlist, (GdkEvent *)event);
        break;
      case GTK_SELECTION_SINGLE:
      case GTK_SELECTION_MULTIPLE:
        if (get_selection_info(hlist, event->x, event->y, &row, &column)) {
...
static gint gtk_hlist_motion(GtkWidget *widget, GdkEventMotion *event) {
  ...
  if (GTK_HLIST_IN_DRAG(hlist)) {
    ...
    if (hlist->column[hlist->drag_pos].max_width >= COLUMN_MIN_WIDTH &&
      ...
      if (COLUMN_LEFT_XPIXEL(hlist, hlist->drag_pos) + new_width >
          ...
          x < 0)
        move_horizontal(hlist, COLUMN_LEFT_XPIXEL(hlist, hlist->drag_pos) +
  ...
  if (GTK_HLIST_REORDERABLE(hlist) && button_actions & GTK_BUTTON_DRAGS) {
    /* delayed drag start */
    if (event->window == hlist->hlist_window && hlist->click_cell.row >= 0 &&
        hlist->click_cell.column >= 0 &&
        (y < 0 || y >= hlist->hlist_window_height || x < 0 ||
      ...
      gtk_drag_begin(widget, target_list, GDK_ACTION_MOVE, hlist->drag_button,
                     (GdkEvent *)event);
    }
    return TRUE;
  }
  ...
  if (hlist->hadjustment && LIST_WIDTH(hlist) > hlist->hlist_window_width &&
    ...
    hlist->htimer =
        gtk_timeout_add(SCROLL_TIME, (GtkFunction)horizontal_timeout, hlist);
  ...
  if (LIST_HEIGHT(hlist) > hlist->hlist_window_height &&
    ...
    hlist->vtimer =
        gtk_timeout_add(SCROLL_TIME, (GtkFunction)vertical_timeout, hlist);

    if (hlist->drag_button && ((y < 0 && hlist->focus_row == 0) ||
  ...
  if (GTK_HLIST_SHOW_TITLES(hlist))
    for (i = 0; i < hlist->columns; i++)
      if (hlist->column[i].button) {
        ...
        gtk_widget_size_request(hlist->column[i].button, &child_requisition);
        hlist->column_title_area.height =
            MAX(hlist->column_title_area.height, child_requisition.height);
      }

  requisition->width +=
      (widget->style->klass->xthickness + GTK_CONTAINER(widget)->border_width) *
      2;
  requisition->height +=
      (hlist->column_title_area.height + (widget->style->klass->ythickness +
...
static void gtk_hlist_size_allocate(GtkWidget *widget,
  ...
  if (GTK_WIDGET_REALIZED(widget)) {
    gdk_window_move_resize(widget->window, allocation->x + border_width,
  ...
  hlist->internal_allocation.width =
      MAX(1, (gint)allocation->width - border_width * 2);
  hlist->internal_allocation.height =
  ...
  hlist_allocation.width =
      MAX(1, (gint)hlist->internal_allocation.width -
                 (2 * (gint)widget->style->klass->xthickness));
  hlist_allocation.height =
      MAX(1, (gint)hlist->internal_allocation.height -
  ...
  if (GTK_WIDGET_REALIZED(widget)) {
    gdk_window_move_resize(hlist->hlist_window, hlist_allocation.x,
                           hlist_allocation.y, hlist_allocation.width,
                           hlist_allocation.height);
  }
  ...
  if (GTK_WIDGET_REALIZED(widget)) {
    gdk_window_move_resize(hlist->title_window, hlist->column_title_area.x,
...
static void draw_row(GtkHList *hlist, GdkRectangle *area, gint row,
  ...
  if (area) {
    ...
    if (gdk_rectangle_intersect(area, &cell_rectangle, &intersect_rectangle))
      gdk_draw_rectangle(hlist->hlist_window,
                         ...
                         intersect_rectangle.x, intersect_rectangle.y,
    ...
    if (hlist_row == hlist->row_list_end->data) {
      ...
      if (gdk_rectangle_intersect(area, &cell_rectangle, &intersect_rectangle))
        gdk_draw_rectangle(
            hlist->hlist_window, widget->style->base_gc[GTK_STATE_ACTIVE], TRUE,
            intersect_rectangle.x, intersect_rectangle.y,
            intersect_rectangle.width, intersect_rectangle.height);
    }
    ...
    gdk_draw_rectangle(hlist->hlist_window,
                       widget->style->base_gc[GTK_STATE_ACTIVE], TRUE,
                       cell_rectangle.x, cell_rectangle.y, cell_rectangle.width,
  ...
  for (i = 0; i < hlist->columns; i++) {
    ...
    clip_rectangle.width +=
        (2 * COLUMN_INSET + HELL_SPACING + (i == last_column) * HELL_SPACING);

    if (area &&
        !gdk_rectangle_intersect(area, &clip_rectangle, &intersect_rectangle))
      continue;

    gdk_window_clear_area(hlist->hlist_window, rect->x, rect->y, rect->width,
                          rect->height);

    if (hlist->stipple_gc && bg_gc == widget->style->bg_gc[GTK_STATE_SELECTED])
      gdk_draw_rectangle(hlist->hlist_window, hlist->stipple_gc, TRUE, rect->x,
                         rect->y, rect->width, rect->height);
    else
      gdk_draw_rectangle(hlist->hlist_window, bg_gc, TRUE, rect->x, rect->y,
    ...
    case GTK_HELL_PIXTEXT:
      gdk_window_get_size(GTK_HELL_PIXTEXT(hlist_row->cell[i])->pixmap,
                          &pixmap_width, &height);
      width = (pixmap_width + GTK_HELL_PIXTEXT(hlist_row->cell[i])->spacing +
               gdk_string_width(style->font,
    ...
    case GTK_HELL_PIXMAP:
      ...
      draw_cell_pixmap(hlist->hlist_window, &clip_rectangle, fg_gc, fill_gc,
                       ...
                       GTK_HELL_PIXMAP(hlist_row->cell[i])->mask, offset,
    ...
    case GTK_HELL_PIXTEXT:
      ...
      draw_cell_pixmap(hlist->hlist_window, &clip_rectangle, fg_gc, fill_gc,
                       ...
                       GTK_HELL_PIXTEXT(hlist_row->cell[i])->mask, offset,
                       clip_rectangle.y + hlist_row->cell[i].vertical +
                           (clip_rectangle.height - height) / 2,
                       pixmap_width, height);
    ...
    case GTK_HELL_TEXT:
      if (style != GTK_WIDGET(hlist)->style)
        row_center_offset = (((hlist->row_height - style->font->ascent -
                               style->font->descent - 1) /
                              2) +
                             1.5 + style->font->ascent);
      else
        row_center_offset = hlist->row_center_offset;
      gdk_gc_set_clip_rectangle(fg_gc, &clip_rectangle);
      gdk_draw_string(hlist->hlist_window, style->font, fg_gc, offset,
  ...
  if (hlist->focus_row == row && GTK_WIDGET_CAN_FOCUS(widget) &&
...
static void draw_rows(GtkHList *hlist, GdkRectangle *area) {
  ...
  if (hlist->row_height == 0 || !GTK_WIDGET_DRAWABLE(hlist))
    return;
  ...
  if (!area)
    gdk_window_clear_area(hlist->hlist_window, 0, ROW_TOP_YPIXEL(hlist, i), 0,
                          0);
}
...
static void draw_xor_line(GtkHList *hlist) {
  ...
  gdk_draw_line(widget->window, hlist->xor_gc, hlist->x_drag,
                widget->style->klass->ythickness, hlist->x_drag,
                hlist->column_title_area.height + hlist->hlist_window_height +
                    1);
}
...
static void adjust_adjustments(GtkHList *hlist, gboolean block_resize) {
  if (hlist->vadjustment) {
    ...
    if (hlist->hlist_window_height - hlist->voffset > LIST_HEIGHT(hlist) ||
      ...
      hlist->vadjustment->value =
          MAX(0, (LIST_HEIGHT(hlist) - hlist->hlist_window_height));
      gtk_signal_emit_by_name(GTK_OBJECT(hlist->vadjustment), "value_changed");
    }
  ...
  if (hlist->hadjustment) {
    ...
    if (hlist->hlist_window_width - hlist->hoffset > LIST_WIDTH(hlist) ||
      ...
      hlist->hadjustment->value =
          MAX(0, (LIST_WIDTH(hlist) - hlist->hlist_window_width));
      gtk_signal_emit_by_name(GTK_OBJECT(hlist->hadjustment), "value_changed");
    }
  ...
  if (value > -hlist->voffset) {
    ...
    if ((diff != 0) && (diff != hlist->hlist_window_height))
      gdk_window_copy_area(
          hlist->hlist_window, hlist->fg_gc, 0, 0, hlist->hlist_window, 0, diff,
          hlist->hlist_window_width, hlist->hlist_window_height - diff);
    ...
    if ((diff != 0) && (diff != hlist->hlist_window_height))
      gdk_window_copy_area(hlist->hlist_window, hlist->fg_gc, 0, diff,
                           hlist->hlist_window, 0, 0, hlist->hlist_window_width,
...
static void hadjustment_value_changed(GtkAdjustment *adjustment,
  ...
  if (value > -hlist->hoffset) {
    ...
    if (GTK_WIDGET_CAN_FOCUS(hlist) && GTK_WIDGET_HAS_FOCUS(hlist) &&
      ...
      gdk_draw_rectangle(hlist->hlist_window, hlist->xor_gc, FALSE, 0, y,
                         hlist->hlist_window_width - 1, hlist->row_height - 1);
    }
    gdk_window_copy_area(
        hlist->hlist_window, hlist->fg_gc, 0, 0, hlist->hlist_window, diff, 0,
        hlist->hlist_window_width - diff, hlist->hlist_window_height);
    ...
    gdk_window_copy_area(
        hlist->hlist_window, hlist->fg_gc, diff, 0, hlist->hlist_window, 0, 0,
        hlist->hlist_window_width - diff, hlist->hlist_window_height);
  ...
  if (GTK_WIDGET_CAN_FOCUS(hlist) && GTK_WIDGET_HAS_FOCUS(hlist) &&
    ...
    if (GTK_HLIST_ADD_MODE(hlist)) {
      ...
      y = ROW_TOP_YPIXEL(hlist, hlist->focus_row);
      gdk_draw_line(hlist->hlist_window, hlist->xor_gc, x0, y + 1, x0,
                    y + hlist->row_height - 2);
      gdk_draw_line(hlist->hlist_window, hlist->xor_gc, x1, y + 1, x1,
                    y + hlist->row_height - 2);
    }
  }
  draw_rows(hlist, &area);
...
static gint gtk_hlist_focus(GtkContainer *container,
  ...
  case GTK_DIR_TAB_BACKWARD:
    if (!focus_child && GTK_HLIST_CHILD_HAS_FOCUS(hlist) && hlist->rows) {
      if (hlist->focus_row < 0) {
        hlist->focus_row = 0;
        if ((hlist->selection_mode == GTK_SELECTION_BROWSE ||
             hlist->selection_mode == GTK_SELECTION_EXTENDED) &&
...
static void gtk_hlist_draw_focus(GtkWidget *widget) {
  ...
  hlist = GTK_HLIST(widget);
  if (hlist->focus_row >= 0)
    gdk_draw_rectangle(hlist->hlist_window, hlist->xor_gc, FALSE, 0,
                       ROW_TOP_YPIXEL(hlist, hlist->focus_row),
                       hlist->hlist_window_width - 1, hlist->row_height - 1);
}
    ...
    while (i < hlist->columns) {
      if (hlist->column[i].button == focus_child) {
        if (hlist->column[i].button &&
          ...
          if (gtk_container_focus(GTK_CONTAINER(hlist->column[i].button),
                                  dir)) {
            return_val = TRUE;
            i -= d;
          }
    ...
    while (j >= 0 && j < hlist->columns) {
      if (hlist->column[j].button &&
        ...
        if (GTK_IS_CONTAINER(hlist->column[j].button) &&
            gtk_container_focus(GTK_CONTAINER(hlist->column[j].button), dir)) {
          return_val = TRUE;
          break;
...
static void move_focus_row(GtkHList *hlist, GtkScrollType scroll_type,
  ...
  case GTK_SCROLL_PAGE_BACKWARD:
    ...
    gtk_hlist_draw_focus(widget);
    hlist->focus_row =
        MAX(0, hlist->focus_row - (2 * hlist->hlist_window_height -
  ...
  case GTK_SCROLL_PAGE_FORWARD:
    ...
    hlist->focus_row =
        MIN(hlist->rows - 1,
            hlist->focus_row + (2 * hlist->hlist_window_height -
                                hlist->row_height - HELL_SPACING) /
                                   (2 * (hlist->row_height + HELL_SPACING)));
    gtk_hlist_draw_focus(widget);
    break;
...
static void scroll_horizontal(GtkHList *hlist, GtkScrollType scroll_type,
  ...
  case GTK_SCROLL_STEP_FORWARD:
    ...
    if (column < 0)
      return;
    if (COLUMN_LEFT_XPIXEL(hlist, column) + hlist->column[column].area.width +
                HELL_SPACING + COLUMN_INSET - 1 <=
            hlist->hlist_window_width &&
        column < last_column)
      column++;
    break;
...
void gtk_hlist_set_compare_func(GtkHList *hlist, GtkHListCompareFunc cmp_func) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_auto_sort(GtkHList *hlist, gboolean auto_sort) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_sort_type(GtkHList *hlist, GtkSortType sort_type) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_sort_column(GtkHList *hlist, gint column) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
 ...
 *   gtk_hlist_mergesort
 */
static gint default_compare(GtkHList *hlist, gconstpointer ptr1,
                            gconstpointer ptr2) {
  char *text1 = NULL;
  char *text2 = NULL;
  ...
  while (a || b) {
    ...
    } else /* a && b */
    {
      ...
      if ((cmp >= 0 && hlist->sort_type == GTK_SORT_DESCENDING) ||
          (cmp <= 0 && hlist->sort_type == GTK_SORT_ASCENDING) || (a && !b)) {
        c->next = a;
        a->prev = c;
        c = a;
        a = a->next;
                                  ...
                                  gint num)    /* the list's length */
{
  ...
  if (num == 1) {
    ...
    return gtk_hlist_merge(hlist, gtk_hlist_mergesort(hlist, list, num / 2),
...
static void drag_source_info_destroy(gpointer data) {
...
static void drag_dest_cell(GtkHList *hlist, gint x, gint y,
  ...
  y -= (GTK_CONTAINER(hlist)->border_width + widget->style->klass->ythickness +
        hlist->column_title_area.height);
...
static void gtk_hlist_drag_leave(GtkWidget *widget, GdkDragContext *context,
  ...
  if (dest_info) {
    if (dest_info->cell.row >= 0 && GTK_HLIST_REORDERABLE(hlist) &&
      ...
      while (list) {
        if (atom == (GdkAtom)GPOINTER_TO_INT(list->data)) {
          GTK_HLIST_CLASS_FW(hlist)->draw_drag_highlight(
              hlist, g_list_nth(hlist->row_list, dest_info->cell.row)->data,
...
static void draw_drag_highlight(GtkHList *hlist, GtkHListRow *dest_row,
  ...
  case GTK_HLIST_DRAG_BEFORE:
    gdk_draw_line(hlist->hlist_window, hlist->xor_gc, 0, y,
                  hlist->hlist_window_width, y);
    break;
...
void gtk_hlist_set_reorderable(GtkHList *hlist, gboolean reorderable) {
  GtkWidget *widget;
...
void gtk_hlist_set_use_drag_icons(GtkHList *hlist, gboolean use_icons) {
  g_return_if_fail(hlist != NULL);
  g_return_if_fail(GTK_IS_HLIST(hlist));
...
void gtk_hlist_set_button_actions(GtkHList *hlist, guint button,
