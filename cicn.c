...
static RGBColor rgb_2[4] = {{0xffff, 0xffff, 0xffff},
                            {0xffff, 0xffff, 0x0000},
                            {0x0000, 0xffff, 0xffff},
                            {0x0000, 0x0000, 0x0000}};

static RGBColor rgb_1[2] = {{0xffff, 0xffff, 0xffff}, {0x0000, 0x0000, 0x0000}};

void palette_get_rgb_8(unsigned int *palette) {
  unsigned int i;

  for (i = 0; i < 256; i++) {
    palette[i] = ((rgb_8[i].blue >> 8) << 16) | ((rgb_8[i].green >> 8) << 8) |
                 ((rgb_8[i].red >> 8) << 0) | 0xff000000;
  }
}
...
static unsigned int rgb_to_pixel(GdkColormap *colormap, RGBColor *rgb) {
  GdkColor col;
...
GdkImage *cicn_to_gdkimage(GdkColormap *colormap, GdkVisual *visual,
                           void *cicn_rsrc, unsigned int len,
                           GdkImage **maskimp) {
  GdkImage *gim;
  GdkImage *mim;
  ...
  ColorTable *ct =
      (ColorTable *)((unsigned char *)cicn_rsrc + 50 + 14 + 14 + 4 +
                     (ntohs(mbm->rowBytes) *
                      (ntohs(mbm->bounds.bottom) - ntohs(mbm->bounds.top))) +
                     (ntohs(bm->rowBytes) *
                      (ntohs(bm->bounds.bottom) - ntohs(bm->bounds.top))));
