...
void DDUP(size_t hi __attribute__((__unused__)),
          size_t i __attribute__((__unused__))) {}

static inline void DDEL(void *ptr) {
...
static inline void DNEW(void *ptr, size_t size, void *oldptr) {
  unsigned long addr = (unsigned long)ptr;
  unsigned long hi = (addr & ((HASHSIZE * ADDRSIZE) - 1)) / ADDRSIZE;
  struct mal_ent *mal_tbl;
  ...
  for (i = 0; i < tbl_size; i++) {
    if (mal_tbl[i].ptr == ptr) {
      if (!oldptr) {
        hxd_log("DNEW: duplicate ptr %p %u %u", ptr, size, mal_tbl[i].size);
        DDUP(hi, i);
      }
      break;
    }
