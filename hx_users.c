...
void hx_user_delete(struct hx_user **utailp, struct hx_user *user) {
  if (user->next)
...
struct hx_user *hx_user_with_uid(struct hx_user *ulist, u_int32_t uid) {
...
struct hx_user *hx_user_with_name(struct hx_user *ulist, const char *name) {
  ...
  for (userp = ulist->next; userp; userp = userp->next) {
    if (!strncmp(userp->name, name, nlen)) {
      if (user) {
        ...
        if (!((uplen == nlen && ulen != nlen) ||
              (ulen == nlen && uplen != nlen)))
          return 0;
      }
      user = userp;
    }
