...
#define GTK_HLIST_CLASS(klass)                                                 \
  (GTK_CHECK_CLASS_CAST((klass), GTK_TYPE_HLIST, GtkHListClass))
#define GTK_IS_HLIST(obj) (GTK_CHECK_TYPE((obj), GTK_TYPE_HLIST))
#define GTK_IS_HLIST_CLASS(klass)                                              \
  (GTK_CHECK_CLASS_TYPE((klass), GTK_TYPE_HLIST))

#define GTK_HLIST_FLAGS(hlist) (GTK_HLIST(hlist)->flags)
#define GTK_HLIST_SET_FLAG(hlist, flag) (GTK_HLIST_FLAGS(hlist) |= (GTK_##flag))
#define GTK_HLIST_UNSET_FLAG(hlist, flag)                                      \
  (GTK_HLIST_FLAGS(hlist) &= ~(GTK_##flag))

#define GTK_HLIST_IN_DRAG(hlist) (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_IN_DRAG)
#define GTK_HLIST_ROW_HEIGHT_SET(hlist)                                        \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_ROW_HEIGHT_SET)
#define GTK_HLIST_SHOW_TITLES(hlist)                                           \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_SHOW_TITLES)
#define GTK_HLIST_CHILD_HAS_FOCUS(hlist)                                       \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_CHILD_HAS_FOCUS)
#define GTK_HLIST_ADD_MODE(hlist) (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_ADD_MODE)
#define GTK_HLIST_AUTO_SORT(hlist)                                             \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_AUTO_SORT)
#define GTK_HLIST_AUTO_RESIZE_BLOCKED(hlist)                                   \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_AUTO_RESIZE_BLOCKED)
#define GTK_HLIST_REORDERABLE(hlist)                                           \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_REORDERABLE)
#define GTK_HLIST_USE_DRAG_ICONS(hlist)                                        \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_USE_DRAG_ICONS)
#define GTK_HLIST_DRAW_DRAG_LINE(hlist)                                        \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_DRAW_DRAG_LINE)
#define GTK_HLIST_DRAW_DRAG_RECT(hlist)                                        \
  (GTK_HLIST_FLAGS(hlist) & GTK_HLIST_DRAW_DRAG_RECT)
...
struct _GtkHListClass {
  ...
  void (*set_scroll_adjustments)(GtkHList *hlist, GtkAdjustment *hadjustment,
                                 GtkAdjustment *vadjustment);
  void (*refresh)(GtkHList *hlist);
  void (*select_row)(GtkHList *hlist, gint row, gint column, GdkEvent *event);
  void (*unselect_row)(GtkHList *hlist, gint row, gint column, GdkEvent *event);
  void (*row_move)(GtkHList *hlist, gint source_row, gint dest_row);
  void (*click_column)(GtkHList *hlist, gint column);
  void (*resize_column)(GtkHList *hlist, gint column, gint width);
  ...
  void (*extend_selection)(GtkHList *hlist, GtkScrollType scroll_type,
                           gfloat position, gboolean auto_start_selection);
  void (*scroll_horizontal)(GtkHList *hlist, GtkScrollType scroll_type,
                            gfloat position);
  void (*scroll_vertical)(GtkHList *hlist, GtkScrollType scroll_type,
                          gfloat position);
  void (*toggle_add_mode)(GtkHList *hlist);
  void (*abort_column_resize)(GtkHList *hlist);
  void (*resync_selection)(GtkHList *hlist, GdkEvent *event);
  GList *(*selection_find)(GtkHList *hlist, gint row_number,
                           GList *row_list_element);
  void (*draw_row)(GtkHList *hlist, GdkRectangle *area, gint row,
                   GtkHListRow *hlist_row);
  void (*draw_drag_highlight)(GtkHList *hlist, GtkHListRow *target_row,
                              gint target_row_number, GtkHListDragPos drag_pos);
  void (*clear)(GtkHList *hlist);
  void (*fake_unselect_all)(GtkHList *hlist, gint row);
  void (*sort_list)(GtkHList *hlist);
  gint (*insert_row)(GtkHList *hlist, gint row, gchar *text[]);
  void (*remove_row)(GtkHList *hlist, gint row);
  void (*set_cell_contents)(GtkHList *hlist, GtkHListRow *hlist_row,
                            gint column, GtkHellType type, const gchar *text,
                            guint8 spacing, GdkPixmap *pixmap, GdkBitmap *mask);
  void (*cell_size_request)(GtkHList *hlist, GtkHListRow *hlist_row,
