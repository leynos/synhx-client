...
extern void hx_printf(struct htlc_conn *htlc, struct hx_chat *chat,
                      const char *fmt, ...);
extern void hx_printf_prefix(struct htlc_conn *htlc, struct hx_chat *chat,
                             const char *prefix, const char *fmt, ...);
extern void hx_save(struct htlc_conn *htlc, struct hx_chat *chat,
                    const char *filename);
extern void hx_load(struct htlc_conn *htlc, struct hx_chat *chat,
                    const char *filename);
...
extern void hx_send_chat(struct htlc_conn *htlc, u_int32_t cid,
                         const char *str);
extern void hotline_client_input(struct htlc_conn *htlc, struct hx_chat *chat,
                                 char *str);
extern int hotline_client_tab(struct htlc_conn *htlc, struct hx_chat *chat,
                              char *buffer, int point);

extern int hist_size;

extern void hx_change_name_icon(struct htlc_conn *htlc, const char *name,
                                u_int16_t icon);
...
extern void hx_connect(struct htlc_conn *htlc, const char *serverstr,
                       u_int16_t port, const char *name, u_int16_t icon,
                       const char *login, const char *pass, int secure);
extern void hx_get_news(struct htlc_conn *htlc);
extern void hx_send_msg(struct htlc_conn *htlc, u_int32_t uid, const char *msg,
                        u_int16_t len, void *p);
...
extern void hx_chat_join(struct htlc_conn *htlc, u_int32_t cid,
                         const char *pass, u_int16_t passlen);
extern void hx_chat_invite(struct htlc_conn *htlc, u_int32_t cid,
                           u_int32_t uid);
extern void hx_set_subject(struct htlc_conn *htlc, u_int32_t cid,
                           const char *subject);
...
extern void hx_get_file_info(struct htlc_conn *htlc, const char *path,
                             int text);
extern void hx_file_delete(struct htlc_conn *htlc, const char *path, int text);
extern void hx_file_move(struct htlc_conn *htlc, const char *frompath,
                         const char *topath);
extern void hx_file_link(struct htlc_conn *htlc, const char *frompath,
                         const char *topath);
extern void hx_tracker_list(struct htlc_conn *htlc, struct hx_chat *chat,
                            const char *addrstr, u_int16_t port);
...
extern int expand_path(struct htlc_conn *htlc, struct hx_chat *chat, char *path,
...
extern struct variable *variable_add(void *ptr, void (*set_fn)(),
                                     const char *nam);
extern void variable_set(struct htlc_conn *htlc, struct hx_chat *chat,
...
extern struct task *task_new(struct htlc_conn *htlc, void (*rcv)(), void *ptr,
...
static inline void strip_ansi(char *buf, int len) {
  register char *p, *end;
...
extern struct hx_user *hx_user_with_name(struct hx_user *ulist,
                                         const char *name);
...
struct output_functions {
  ...
  void (*chat)(struct htlc_conn *htlc, u_int32_t cid, char *chat,
               u_int16_t len);
  void (*chat_subject)(struct htlc_conn *htlc, u_int32_t cid,
                       const char *subject);
  void (*chat_password)(struct htlc_conn *htlc, u_int32_t cid,
                        const char *pass);
  void (*chat_invite)(struct htlc_conn *htlc, u_int32_t cid, u_int32_t uid,
                      const char *name);
  void (*chat_delete)(struct htlc_conn *htlc, struct hx_chat *chat);
  void (*msg)(struct htlc_conn *htlc, u_int32_t uid, const char *name,
              const char *msgbuf, u_int16_t msglen);
  void (*agreement)(struct htlc_conn *htlc, const char *agreement,
                    u_int16_t len);
  ...
  void (*user_info)(struct htlc_conn *htlc, u_int32_t uid, const char *nam,
                    const char *info, u_int16_t len);
  void (*user_create)(struct htlc_conn *htlc, struct hx_chat *chat,
                      struct hx_user *user, const char *nam, u_int16_t icon,
                      u_int16_t color);
  void (*user_delete)(struct htlc_conn *htlc, struct hx_chat *chat,
                      struct hx_user *user);
  void (*user_change)(struct htlc_conn *htlc, struct hx_chat *chat,
                      struct hx_user *user, const char *nam, u_int16_t icon,
                      u_int16_t color);
  ...
  void (*file_info)(struct htlc_conn *htlc, const char *icon, const char *type,
                    const char *crea, u_int32_t size, const char *name,
                    const char *created, const char *modified,
                    const char *comment);
  void (*file_update)(struct htxf_conn *htxf);
  void (*tracker_server_create)(struct htlc_conn *htlc, const char *addrstr,
