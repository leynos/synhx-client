...
static void writestuff(const char *str, u_int8_t type, const u_int8_t *buf,
                       unsigned int len) {
  unsigned int i;
  char file[32];
  FILE *fp;
...
u_int32_t cipher_decode(struct htlc_conn *htlc, struct qbuf *out,
                        struct qbuf *in, u_int32_t max, u_int32_t *inusedp) {
  u_int32_t len;
  ...
  case CIPHER_RC4:
    ...
    rc4_buffer(&out->buf[out->pos], len, &htlc->cipher_decode_state.rc4);
    break;
  case CIPHER_BLOWFISH:
    BF_ofb64_encrypt(&in->buf[in->pos], &out->buf[out->pos], len,
                     &htlc->cipher_decode_state.blowfish.state,
                     htlc->cipher_decode_state.blowfish.ivec,
                     &htlc->cipher_decode_state.blowfish.num);
    break;
  case CIPHER_IDEA:
    idea_ofb64_encrypt(&in->buf[in->pos], &out->buf[out->pos], len,
...
void cipher_change_decode_key(struct htlc_conn *htlc, u_int32_t type) {
  u_int16_t len = 0;
  u_int32_t i, num;

  num = (type >> 24) & 0xff;
  for (i = 0; i < num; i++) {
    len = hmac_xxx(htlc->cipher_decode_key, htlc->cipher_decode_key,
                   htlc->cipher_decode_keylen, htlc->sessionkey, htlc->sklen,
                   htlc->macalg);
  }
#if CIPHER_DEBUG
...
static void cipher_change_encode_key(struct htlc_conn *htlc, unsigned int num) {
  u_int16_t len = 0;
  unsigned int i;

  for (i = 0; i < num; i++) {
    len = hmac_xxx(htlc->cipher_encode_key, htlc->cipher_encode_key,
                   htlc->cipher_encode_keylen, htlc->sessionkey, htlc->sklen,
                   htlc->macalg);
  }
#if CIPHER_DEBUG
...
static void do_encode(struct htlc_conn *htlc, unsigned int pos,
                      unsigned int len) {
#if CIPHER_DEBUG
  ...
  case CIPHER_RC4:
    rc4_buffer(&htlc->out.buf[pos], len, &htlc->cipher_encode_state.rc4);
    break;
  case CIPHER_BLOWFISH:
    BF_ofb64_encrypt(&htlc->out.buf[pos], &htlc->out.buf[pos], len,
                     &htlc->cipher_encode_state.blowfish.state,
                     htlc->cipher_encode_state.blowfish.ivec,
                     &htlc->cipher_encode_state.blowfish.num);
    break;
  case CIPHER_IDEA:
    idea_ofb64_encrypt(&htlc->out.buf[pos], &htlc->out.buf[pos], len,
...
void cipher_encode(struct htlc_conn *htlc, unsigned int pos, unsigned int len) {
...
void cipher_encode_init(struct htlc_conn *htlc) {
  switch (htlc->cipher_encode_type) {
  case CIPHER_RC4:
...
void cipher_decode_init(struct htlc_conn *htlc) {
  switch (htlc->cipher_decode_type) {
  case CIPHER_RC4:
